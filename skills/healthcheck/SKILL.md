---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Donna deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Donna cron scheduling for periodic checks, or version status checks on a machine running Donna (laptop, workstation, Pi, VPS).
---

# Donna Host Hardening

## Overview

Assess and harden the host running Donna, then align it to a user-defined risk tolerance without breaking access. Use Donna security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Donna changes the host firewall, SSH, or OS updates; it does not.
...
5. Donna gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Donna can access sensitive data.
...
### 2) Run Donna security audits (read-only)

As part of the default read-only checks, run `donna security audit --deep`. Only offer alternatives if the user requests them:

1. `donna security audit` (faster, non-probing)
2. `donna security audit --json` (structured output)

Offer to apply Donna safe defaults (numbered):

1. `donna security audit --fix`

Be explicit that `--fix` only tightens Donna defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Donna version/update status (read-only)

As part of the default read-only checks, run `donna update status`.
...
- Credential hygiene notes (location of Donna creds, prefer disk encryption)
...
After Donna install or first hardening pass, run at least one baseline audit and version check:

- `donna security audit`
- `donna security audit --deep`
- `donna update status`

Ongoing monitoring is recommended. Use the Donna cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `donna cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `donna update status`
...
Before creating, `donna cron list` and match on exact `name`. If found, `donna cron edit <id> ...`.
If not found, `donna cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `donna update status` (preferred for source checkouts and channels)
2. `npm view donna version` (published npm version)

## Donna command accuracy

Use only supported commands and flags:

- `donna security audit [--deep] [--fix] [--json]`
- `donna status` / `donna status --deep`
- `donna health --json`
- `donna update status`
- `donna cron add|list|runs|run`

Do not invent CLI flags or imply Donna enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Donna compaction:
