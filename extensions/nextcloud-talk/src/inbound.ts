import {
  ...
  type DonnaConfig,
  type RuntimeEnv,
} from "donna/plugin-sdk/nextcloud-talk";
...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as DonnaConfig);
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as DonnaConfig,
    surface: CHANNEL_ID,
  });
  const useAccessGroups =
    (config.commands as Record<string, unknown> | undefined)?.useAccessGroups !== false;
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as DonnaConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as DonnaConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as DonnaConfig);
