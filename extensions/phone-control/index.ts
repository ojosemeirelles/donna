...
import type { DonnaPluginApi, DonnaPluginService } from "donna/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: DonnaPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: DonnaPluginApi["config"]): string[] {
...
export default function register(api: DonnaPluginApi) {
  ...
  const timerService: DonnaPluginService = {
