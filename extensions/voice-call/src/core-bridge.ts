...
function resolveDonnaRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.DONNA_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set DONNA_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveDonnaRoot(), "dist", "extensionAPI.js");
