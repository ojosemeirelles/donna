...
import type { DonnaPluginApi, DonnaPluginToolContext } from "donna/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<DonnaPluginApi> = {}): DonnaPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<DonnaPluginToolContext> = {}): DonnaPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "donna-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: DonnaPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
