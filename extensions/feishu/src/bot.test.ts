import type { DonnaConfig, PluginRuntime, RuntimeEnv } from "donna/plugin-sdk/feishu";
...
async function dispatchMessage(params: { cfg: DonnaConfig; event: FeishuMessageEvent }) {
...
describe("resolveBroadcastAgents", () => {
  it("returns agent list when broadcast config has the peerId", () => {
    const cfg = { broadcast: { oc_group123: ["susan", "main"] } } as unknown as DonnaConfig;
  ...
  it("returns null when peerId not in broadcast", () => {
    const cfg = { broadcast: { oc_other: ["susan"] } } as unknown as DonnaConfig;
    expect(resolveBroadcastAgents(cfg, "oc_group123")).toBeNull();
  });

  it("returns null when agent list is empty", () => {
    const cfg = { broadcast: { oc_group123: [] } } as unknown as DonnaConfig;
    expect(resolveBroadcastAgents(cfg, "oc_group123")).toBeNull();
  });
});
