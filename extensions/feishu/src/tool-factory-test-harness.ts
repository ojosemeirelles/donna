import type { AnyAgentTool, DonnaPluginApi } from "donna/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: DonnaPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<DonnaPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
