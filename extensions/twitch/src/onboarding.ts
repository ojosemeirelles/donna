 ...
 */
function setTwitchAccount(cfg: DonnaConfig, account: Partial<TwitchAccountConfig>): DonnaConfig {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: DONNA_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: DonnaConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var DONNA_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.DONNA_TWITCH_ACCESS_TOKEN?.trim();
