...
This extension depends on Donna internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Donna extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.donna/extensions` as a standalone plugin directory.
