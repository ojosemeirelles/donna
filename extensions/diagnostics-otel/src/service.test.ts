...
vi.mock("donna/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("donna/plugin-sdk/diagnostics-otel")>(
    "donna/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { DonnaPluginServiceContext } from "donna/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "donna/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/donna-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): DonnaPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("donna.webhook.received")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("donna.webhook.duration_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("donna.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("donna.message.processed")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("donna.message.duration_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.histograms.get("donna.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("donna.session.stuck")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("donna.session.stuck_age_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("donna.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("donna.webhook.processed");
    expect(spanNames).toContain("donna.message.processed");
    expect(spanNames).toContain("donna.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["donna.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("donna.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "donna.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["donna.reason"]).toBe("string");
    expect(String(attrs?.["donna.reason"])).not.toContain("ghp_abcdefghijklmnopqrstuvwxyz123456");
    await service.stop?.(ctx);
  });
});
