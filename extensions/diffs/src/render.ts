...
function renderDiffCard(payload: DiffViewerPayload): string {
  return `<section class="oc-diff-card">
    <diffs-container class="oc-diff-host" data-donna-diff-host>
      <template shadowrootmode="open">${payload.prerenderedHTML}</template>
    </diffs-container>
    <script type="application/json" data-donna-diff-payload>${escapeJsonScript(payload)}</script>
  </section>`;
}
...
function buildHtmlDocument(params: {
  ...
  return `<!doctype html>
<html lang="en"${params.runtimeMode === "image" ? ' data-donna-diffs-ready="true"' : ""}>
  <head>
    <meta charset="utf-8" />
