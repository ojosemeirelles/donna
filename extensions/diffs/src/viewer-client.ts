...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-donna-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-donna-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.donnaDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.donnaDiffsError = "true";
