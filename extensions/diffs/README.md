...
Read-only diff viewer plugin for **Donna** agents.
...
Set plugin-wide defaults in `~/.donna/donna.json`:
...
Before:
Donna supports plugins.

After:
Donna supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/donna-diffs`).
