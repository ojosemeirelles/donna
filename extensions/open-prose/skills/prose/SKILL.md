...
metadata: { "donna": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Donna `sessions_spawn`
- **File I/O** == Donna `read`/`write`
- **Remote fetch** == Donna `web_fetch` (or `exec` with curl when POST is required)
