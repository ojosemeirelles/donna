...
This document describes how the Donna ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`donna acp` exposes an ACP agent over stdio and forwards prompts to a running
Donna Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `donna acp` and speaks ACP messages over stdio.
...
`donna acp` resolves the Gateway URL and auth from CLI flags or config:
