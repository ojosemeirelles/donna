...
Donna supports **camera capture** for agent workflows:
...
```bash
donna nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
donna nodes camera snap --node <id> --facing front
donna nodes camera clip --node <id> --duration 3000
donna nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`donna.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `donna` CLI to invoke camera commands on the macOS node.

Examples:

```bash
donna nodes camera list --node <id>            # list camera ids
donna nodes camera snap --node <id>            # prints MEDIA:<path>
donna nodes camera snap --node <id> --max-width 1280
donna nodes camera snap --node <id> --delay-ms 2000
donna nodes camera snap --node <id> --device-id <id>
donna nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
donna nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
donna nodes camera clip --node <id> --device-id <id>
donna nodes camera clip --node <id> --no-audio
```

Notes:

- `donna nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
donna nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
