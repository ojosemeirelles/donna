...
read_when:
  ...
  - Wiring external systems into Donna
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Donna config directory (typically `~/.donna/hooks/transforms`).
...
- `donna webhooks gmail setup` writes `hooks.gmail` config for `donna webhooks gmail run`.
