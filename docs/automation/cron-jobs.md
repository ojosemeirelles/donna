...
- Jobs persist under `~/.donna/cron/` so restarts don’t lose schedules.
...
donna cron list
donna cron run <job-id>
donna cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.donna/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `donna cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Donna applies a
...
If `delivery` is omitted for isolated jobs, Donna defaults to `announce`.
...
- Job store: `~/.donna/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.donna/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Donna classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, Donna archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `donna cron runs` and adjust retention before logs become large
...
donna cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
donna cron edit <jobId> --agent ops
donna cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
donna cron run <jobId>
donna cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
donna cron edit <jobId> \
...
```bash
donna system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`donna system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `DONNA_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Donna applies exponential retry backoff for recurring jobs after consecutive errors:
