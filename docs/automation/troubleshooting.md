...
donna system heartbeat last
donna logs --follow
donna config get agents.defaults.heartbeat
donna channels status --probe
```
...
```bash
donna config get agents.defaults.heartbeat.activeHours
donna config get agents.defaults.heartbeat.activeHours.timezone
donna config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
donna cron list
donna logs --follow
```
