---
summary: "Donna on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Donna on DigitalOcean
  - Looking for cheap VPS hosting for Donna
title: "DigitalOcean"
---

# Donna on DigitalOcean

## Goal

Run a persistent Donna Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status donna-gateway.service

# View logs
journalctl --user -u donna-gateway.service -f
```
...
- `~/.donna/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u donna --no-pager -n 50
```
