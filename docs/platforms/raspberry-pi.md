---
summary: "Donna on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Donna on a Raspberry Pi
  - Running Donna on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Donna on Raspberry Pi

## Goal

Run a persistent, always-on Donna Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
donna config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/donna-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
export NODE_COMPILE_CACHE=/var/tmp/donna-compile-cache
mkdir -p /var/tmp/donna-compile-cache
export DONNA_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `DONNA_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Donna, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit donna
```

```ini
[Service]
Environment=DONNA_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/donna-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Donna state/cache on SSD-backed storage to avoid SD-card
...
Most Donna features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled donna

# Enable if not
sudo systemctl enable donna

# Start on boot
sudo systemctl start donna
```
...
```bash
# Check logs
journalctl -u donna --no-pager -n 100

# Common fix: rebuild
cd ~/donna  # if using hackable install
npm run build
sudo systemctl restart donna
```
