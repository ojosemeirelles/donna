...
service via the external `donna` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.donna.gateway`
  (or `ai.donna.<profile>` when using `--profile`/`DONNA_PROFILE`; legacy `com.donna.*` is supported).
