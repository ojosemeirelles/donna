...
- Same path: run the app from a fixed location (for Donna, `dist/Donna.app`).
...
```bash
sudo tccutil reset Accessibility ai.donna.mac
sudo tccutil reset ScreenCapture ai.donna.mac
...
Workaround: move files into the Donna workspace (`~/.donna/workspace`) if you want to avoid per-folder grants.
