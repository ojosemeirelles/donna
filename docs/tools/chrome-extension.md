---
summary: "Chrome extension: let Donna drive your existing Chrome tab"
read_when:
...
The Donna Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate donna-managed Chrome profile.
...
Donna then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Donna release (npm package) as static files. There is no separate “build” step.

After upgrading Donna:

- Re-run `donna browser extension install` to refresh the installed files under your Donna state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Donna ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `DONNA_GATEWAY_TOKEN`)
...
- Open the tab you want Donna to control.
...
- `ON`: attached; Donna can drive that tab.
...
Debugging: `donna sandbox explain`
...
`donna browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `donna browser extension install` first to copy the extension to a stable location under your Donna state directory.
...
- **This is not isolated** like the dedicated donna-managed profile.
