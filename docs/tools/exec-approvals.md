...
- **gateway host** → `donna` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.donna/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.donna/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `donna approvals allowlist ...`).
- `donna security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `donna doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.donna/exec-approvals.json` directly.

CLI: `donna approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
