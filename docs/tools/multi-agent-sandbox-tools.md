...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `donna sandbox explain`.
...
Legacy `agent.*` configs are migrated by `donna doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=donna-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${DONNA_STATE_DIR:-$HOME/.donna}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
