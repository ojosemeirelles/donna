---
summary: "CLI reference for `donna onboard` (interactive onboarding wizard)"
read_when:
...
```bash
donna onboard
donna onboard --flow quickstart
donna onboard --flow manual
donna onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`DONNA_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `donna dashboard` (Control UI, no channel setup).
