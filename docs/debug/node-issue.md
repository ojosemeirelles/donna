...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Donna
...
Running Donna via Node with `tsx` fails at startup with:

```
[donna] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `donna status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch donna.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node donna.mjs status` works on Node 25.
