...
Tlon is a decentralized messenger built on Urbit. Donna connects to your Urbit ship and can
...
```bash
donna plugins install @donna/tlon
```

Local checkout (when running from a git repo):

```bash
donna plugins install ./extensions/tlon
```
...
By default, Donna blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `donna message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Donna replies in-thread.
