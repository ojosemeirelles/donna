...
read_when:
  - You want Donna to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Donna to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`donna onboard`) and `donna channels add` list optional channel plugins.
...
```bash
donna plugins install @donna/nostr
```

Use a local checkout (dev workflows):

```bash
donna plugins install --link <path-to-donna>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "donna@example.com",
        "lud16": "donna@example.com"
      }
    }
  }
}
