...
```bash
donna plugins install @donna/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
donna plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Donna" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `donna pairing list nextcloud-talk`
  - `donna pairing approve nextcloud-talk <CODE>`
