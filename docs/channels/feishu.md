...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Donna to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
donna plugins install @donna/feishu
```

Local checkout (when running from a git repo):

```bash
donna plugins install ./extensions/feishu
```
...
If you just installed Donna, run the wizard:
...
2. Configuring app credentials in Donna
...
1. You already ran `donna channels add` for Feishu
2. The gateway is running (`donna gateway status`)
...
```bash
donna pairing approve feishu <CODE>
```
...
2. Run `donna logs --follow` and look for `chat_id`
...
2. Run `donna logs --follow` and look for `open_id`
...
| Command                 | Description                   |
| ----------------------- | ----------------------------- |
| `donna gateway status`  | Show gateway status           |
| `donna gateway install` | Install/start gateway service |
| `donna gateway stop`    | Stop gateway service          |
| `donna gateway restart` | Restart gateway service       |
| `donna logs --follow`   | Tail gateway logs             |
...
4. Check logs: `donna logs --follow`
...
5. Ensure the gateway is running: `donna gateway status`
6. Check logs: `donna logs --follow`
...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "donna-fan",
        workspace: "/home/user/donna-fan",
        agentDir: "/home/user/.donna/agents/donna-fan/agent",
      },
      {
        id: "donna-xi",
        workspace: "/home/user/donna-xi",
        agentDir: "/home/user/.donna/agents/donna-xi/agent",
      },
    ],
  },
  bindings: [
