...
Matrix is an open, decentralized messaging protocol. Donna connects as a Matrix **user**
...
```bash
donna plugins install @donna/matrix
```

Local checkout (when running from a git repo):

```bash
donna plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `donna plugins install @donna/matrix`
   - From a local checkout: `donna plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Donna calls the same
     login endpoint, stores the access token in `~/.donna/credentials/matrix/credentials.json`,
...
- On first connection, Donna requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Donna logs a warning.
...
`~/.donna/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `donna pairing list matrix`
  - `donna pairing approve matrix <CODE>`
...
- On startup, Donna resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
