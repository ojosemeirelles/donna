...
read_when:
  - Setting up Zalo Personal for Donna
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Donna.
...
- Install via CLI: `donna plugins install @donna/zalouser`
- Or from a source checkout: `donna plugins install ./extensions/zalouser`
...
- `donna pairing list zalouser`
- `donna pairing approve zalouser <code>`
...
- On startup, Donna resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
Accounts map to `zalouser` profiles in Donna state. Example:
...
- Donna sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Donna sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `donna channels logout --channel zalouser && donna channels login --channel zalouser`
...
- The channel now runs fully in Donna without external CLI binaries.
