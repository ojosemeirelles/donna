...
read_when:
  - Setting up Twitch chat integration for Donna
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Donna connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
donna plugins install @donna/twitch
```

Local checkout (when running from a git repo):

```bash
donna plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `DONNA_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "donna", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use DONNA_TWITCH_ACCESS_TOKEN env var)
...
```bash
DONNA_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
