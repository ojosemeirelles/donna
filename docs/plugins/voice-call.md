...
read_when:
  - You want to place an outbound voice call from Donna
...
Voice calls for Donna via a plugin. Supports outbound notifications and
...
- Use `donna voicecall ...` or the `voice_call` tool
...
```bash
donna plugins install ./extensions/voice-call
