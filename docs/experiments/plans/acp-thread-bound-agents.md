...
This plan defines how Donna should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Donna core owns ACP control plane concerns
...
Donna should not reimplement ACP transport internals in core.
Donna should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Donna, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Donna state dir
...
- ACP event stream is normalized to Donna reply chunks
...
For the first production backend (`extensions/acpx`), Donna and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Donna `sessionKey`)
...
- Donna consumes ndjson events from `acpx --format json --json-strict`
