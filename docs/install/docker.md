---
summary: "Optional Docker-based setup and onboarding for Donna"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Donna on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full Donna in Docker)
...
- builds the gateway image locally (or pulls a remote image if `DONNA_IMAGE` is set)
...
- `DONNA_IMAGE` — use a remote image instead of building locally (e.g. `ghcr.io/donna/donna:latest`)
- `DONNA_DOCKER_APT_PACKAGES` — install extra apt packages during build
- `DONNA_EXTRA_MOUNTS` — add extra host bind mounts
- `DONNA_HOME_VOLUME` — persist `/home/node` in a named volume
- `DONNA_SANDBOX` — opt in to Docker gateway sandbox bootstrap. Only explicit truthy values enable it: `1`, `true`, `yes`, `on`
- `DONNA_INSTALL_DOCKER_CLI` — build arg passthrough for local image builds (`1` installs Docker CLI in the image). `docker-setup.sh` sets this automatically when `DONNA_SANDBOX=1` for local builds.
- `DONNA_DOCKER_SOCKET` — override Docker socket path (default: `DOCKER_HOST=unix://...` path, else `/var/run/docker.sock`)
- `DONNA_ALLOW_INSECURE_PRIVATE_WS=1` — break-glass: allow trusted private-network
  `ws://` targets for CLI/onboarding client paths (default is loopback-only)
- `DONNA_BROWSER_DISABLE_GRAPHICS_FLAGS=0` — disable container browser hardening flags
  `--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu` when you need
  WebGL/3D compatibility.
- `DONNA_BROWSER_DISABLE_EXTENSIONS=0` — keep extensions enabled when browser
  flows require them (default keeps extensions disabled in sandbox browser).
- `DONNA_BROWSER_RENDERER_PROCESS_LIMIT=<N>` — set Chromium renderer process
...
- Need the URL again? Run `docker compose run --rm donna-cli dashboard --no-open`.
...
```bash
export DONNA_SANDBOX=1
export DONNA_DOCKER_SOCKET=/run/user/1000/docker.sock
./docker-setup.sh
```
...
- If `Dockerfile.sandbox` is missing, the script prints a warning and continues;
  build `donna-sandbox:bookworm-slim` with `scripts/sandbox-setup.sh` if
  needed.
- For non-local `DONNA_IMAGE` values, the image must already contain Docker
...
```bash
docker compose run -T --rm donna-cli gateway probe
docker compose run -T --rm donna-cli devices list --json
```
...
`donna-cli` uses `network_mode: "service:donna-gateway"` so CLI commands can
...
network path instead of the bundled `donna-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `donna-cli`.
...
- [GitHub Container Registry package](https://github.com/donna/donna/pkgs/container/donna)

Use image name `ghcr.io/donna/donna` (not similarly named Docker Hub
images).
...
- `org.opencontainers.image.source=https://github.com/donna/donna`
- `org.opencontainers.image.url=https://donna.ai`
- `org.opencontainers.image.documentation=https://docs.donna.ai/install/docker`
- `org.opencontainers.image.licenses=MIT`
- `org.opencontainers.image.title=Donna`
- `org.opencontainers.image.description=Donna gateway and CLI runtime container image`
...
image instead, set `DONNA_IMAGE` before running the script:

```bash
export DONNA_IMAGE="ghcr.io/donna/donna:latest"
./docker-setup.sh
```

The script detects that `DONNA_IMAGE` is not the default `donna:local` and
...
`docker-compose.yml` and helper files. `DONNA_IMAGE` skips local image build
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/donna/donna/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
```
...
See [`ClawDock` Helper README](https://github.com/donna/donna/blob/main/scripts/shell-helpers/README.md) for details.

### Manual flow (compose)

```bash
docker build -t donna:local -f Dockerfile .
docker compose run --rm donna-cli onboard
docker compose up -d donna-gateway
```

Note: run `docker compose ...` from the repo root. If you enabled
`DONNA_EXTRA_MOUNTS` or `DONNA_HOME_VOLUME`, the setup script writes
...
```bash
docker compose run --rm donna-cli dashboard --no-open
docker compose run --rm donna-cli devices list
docker compose run --rm donna-cli devices approve <requestId>
```
...
`DONNA_EXTRA_MOUNTS` before running `docker-setup.sh`. This accepts a
comma-separated list of Docker bind mounts and applies them to both
`donna-gateway` and `donna-cli` by generating `docker-compose.extra.yml`.

Example:

```bash
export DONNA_EXTRA_MOUNTS="$HOME/.codex:/home/node/.codex:ro,$HOME/github:/home/node/github:rw"
./docker-setup.sh
```
...
- If you edit `DONNA_EXTRA_MOUNTS`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
volume via `DONNA_HOME_VOLUME`. This creates a Docker volume and mounts it at
...
```bash
export DONNA_HOME_VOLUME="donna_home"
./docker-setup.sh
```
...
- If you change `DONNA_HOME_VOLUME`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
libraries), set `DONNA_DOCKER_APT_PACKAGES` before running `docker-setup.sh`.
...
```bash
export DONNA_DOCKER_APT_PACKAGES="ffmpeg build-essential"
./docker-setup.sh
```
...
- If you change `DONNA_DOCKER_APT_PACKAGES`, rerun `docker-setup.sh` to rebuild
  the image.
...
```bash
export DONNA_DOCKER_APT_PACKAGES="git curl jq"
./docker-setup.sh
```
...
`DONNA_DOCKER_APT_PACKAGES` instead of using `--with-deps` at runtime.

4. **Persist Playwright browser downloads**:

- Set `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright` in
  `docker-compose.yml`.
- Ensure `/home/node` persists via `DONNA_HOME_VOLUME`, or mount
  `/home/node/.cache/ms-playwright` via `DONNA_EXTRA_MOUNTS`.

### Permissions + EACCES

The image runs as `node` (uid 1000). If you see permission errors on
`/home/node/.donna`, make sure your host bind mounts are owned by uid 1000.

Example (Linux host):

```bash
sudo chown -R 1000:1000 /path/to/donna-config /path/to/donna-workspace
```
...
```bash
docker compose run --rm donna-cli channels login
```

Telegram (bot token):

```bash
docker compose run --rm donna-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):

```bash
docker compose run --rm donna-cli channels add --channel discord --token "<token>"
```
...
background. In plain terms: Docker keeps checking if Donna is still
...
```bash
docker compose exec donna-gateway node dist/index.js health --token "$DONNA_GATEWAY_TOKEN"
```
...
`docker-setup.sh` defaults `DONNA_GATEWAY_BIND=lan` so host access to
...
```bash
docker compose run --rm donna-cli config set gateway.mode local
docker compose run --rm donna-cli config set gateway.bind lan
docker compose run --rm donna-cli devices list --url ws://127.0.0.1:18789
```

### Notes

- Gateway bind defaults to `lan` for container use (`DONNA_GATEWAY_BIND`).
- Dockerfile CMD uses `--allow-unconfigured`; mounted config with `gateway.mode` not `local` will still start. Override CMD to enforce the guard.
- The gateway container is the source of truth for sessions (`~/.donna/agents/<agentId>/sessions/`).
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.donna/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  Donna auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `donna sandbox recreate ...` command.
...
This builds `donna-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `donna-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "donna-sandbox-common:bookworm-slim" } },
    },
  },
}
```
...
This builds `donna-sandbox-browser:bookworm-slim` using
...
- Browser containers default to a dedicated Docker network (`donna-sandbox-browser`) instead of global `bridge`.
...
- noVNC observer access is password-protected by default; Donna provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
- Browser container startup defaults are conservative for shared/container workloads, including:
  ...
  - The three graphics hardening flags above are optional. If your workload needs
    WebGL/3D, set `DONNA_BROWSER_DISABLE_GRAPHICS_FLAGS=0` to run without
  ...
  - Extension behavior is controlled by `--disable-extensions` and can be disabled
    (enables extensions) via `DONNA_BROWSER_DISABLE_EXTENSIONS=0` for
    extension-dependent pages or extensions-heavy workflows.
  - `--renderer-process-limit=2` is also configurable with
    `DONNA_BROWSER_RENDERER_PROCESS_LIMIT`; set `0` to let Chromium choose its
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-donna-browser" } },
    },
  },
}
```
...
```bash
docker build -t my-donna-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-donna-sbx" } },
    },
  },
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/donna/donna/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: Donna runs commands with `sh -lc` (login shell), which
