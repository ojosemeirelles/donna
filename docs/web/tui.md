...
```bash
donna tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `donna tui --deliver`
...
- Local shell commands receive `DONNA_SHELL=tui-local` in their environment.
...
- Check the Gateway logs: `donna logs --follow`.
- Confirm the agent can run: `donna status` and `donna models status`.
...
- No agents in picker: check `donna agents list` and your routing config.
