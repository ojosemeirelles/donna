...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/donna`)
...
If the page fails to load, start the Gateway first: `donna gateway`.
...
you revoke it with `donna devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.donna/donna.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop donna`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Donna
...
Donna **blocks** Control UI connections without device identity.
