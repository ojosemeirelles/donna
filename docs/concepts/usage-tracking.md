...
- `/usage cost` in chats: local cost summary aggregated from Donna session logs.
- CLI: `donna status --usage` prints a full per-provider breakdown.
- CLI: `donna channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
