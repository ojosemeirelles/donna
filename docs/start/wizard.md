...
The onboarding wizard is the **recommended** way to set up Donna on macOS,
...
`donna dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
(`web_fetch` works without a key). Easiest path: `donna configure --section web`
...
2. **Workspace** — Location for agent files (default `~/.donna/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `donna doctor` first.
</Note>
...
Use `donna agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.donna/workspace-<agentId>`.
...
- CLI command reference: [`donna onboard`](/cli/onboard)
