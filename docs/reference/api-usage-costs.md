...
Donna features that can generate provider usage or paid API calls.
...
- `donna status --usage` and `donna channels list` show provider **usage windows**
...
Donna can pick up credentials from:
...
`donna models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
