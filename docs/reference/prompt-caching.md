...
- With Anthropic API-key auth profiles, Donna seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Donna injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Donna exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.donna/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$DONNA_STATE_DIR/logs/cache-trace.jsonl`
...
- `DONNA_CACHE_TRACE=1` enables cache tracing.
- `DONNA_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `DONNA_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `DONNA_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `DONNA_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
