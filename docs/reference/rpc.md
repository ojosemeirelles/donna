...
Donna integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Donna owns lifecycle when `channels.signal.autoStart=true`.
...
- Donna spawns `imsg rpc` as a child process (legacy iMessage integration).
