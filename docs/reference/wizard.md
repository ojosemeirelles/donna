...
This is the full reference for the `donna onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.donna/credentials/oauth.json`; auth profiles live in `~/.donna/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.donna/credentials/oauth.json` (or `$DONNA_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.donna/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `donna pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `donna health`.
    - Tip: `donna status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
- Stores it under `~/.donna/tools/signal-cli/<version>/`.
...
Typical fields in `~/.donna/donna.json`:
...
`donna agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.donna/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.donna/agents/<agentId>/sessions/`.
