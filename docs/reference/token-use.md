---
summary: "How Donna builds prompt context and reports token usage + costs"
read_when:
...
Donna tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Donna assembles its own system prompt on every run. It includes:
...
For images, Donna downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Donna session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `donna status --usage` and `donna channels list` show
...
`cacheWrite`. If pricing is missing, Donna shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Donna can
...
Anthropic's 1M context window is currently beta-gated. Donna can inject the
...
Donna skips the `context-1m-*` beta header because Anthropic currently
