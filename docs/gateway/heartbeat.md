...
- During heartbeat runs, Donna treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `donna sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Donna skips the heartbeat run entirely (no model call).
...
```bash
donna system event --text "Check for urgent follow-ups" --mode now
```
