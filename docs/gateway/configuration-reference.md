...
description: "Complete field-by-field reference for ~/.donna/donna.json"
summary: "Complete reference for every Donna config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.donna/donna.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Donna uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `donna doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `donna doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["donna-dm"] },
      guilds: {
        "123456789012345678": {
...
- Donna additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `donna plugins install @donna/mattermost`.
...
- `commands.callbackUrl` must resolve to the Donna gateway endpoint and be reachable from the Mattermost server.
...
Donna spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `donna channels add` (or channel onboarding) while still on a single-account top-level channel config, Donna moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `donna doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@donna"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `donna.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Donna auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Donna assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.donna/sandboxes`
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `donna.json`.
noVNC observer access uses VNC auth by default and Donna emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `donna-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from DONNA_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `DONNA_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `DONNA_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `DONNA_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.donna/workspace",
        agentDir: "~/.donna/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@donna"] },
        sandbox: { mode: "off" },
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Donna starts a fresh thread session instead of inheriting parent transcript history.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:donna`      | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.donna/attachments/<uuid>/` with a `.manifest.json`.
...
Donna uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.donna/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `DONNA_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Donna forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `donna onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `donna onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `donna onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `donna onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `donna onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `donna onboard --auth-choice minimax-api`.
...
- Loaded from `~/.donna/extensions`, `<workspace>/.donna/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `donna plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or DONNA_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `DONNA_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `DONNA_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
Convenience flags: `--dev` (uses `~/.donna-dev` + port `19001`), `--profile <name>` (uses `~/.donna-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.donna/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-donna-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `DONNA_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or DONNA_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__donna__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__donna__/a2ui/`
...
- Also serves A2UI at `/__donna__/a2ui/`.
...
- Hostname defaults to `donna`. Override with `DONNA_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.donna/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `donna dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.donna/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${DONNA_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `donna.json` credential paths.
...
- Legacy OAuth imports from `~/.donna/credentials/oauth.json`.
...
- Default log file: `/tmp/donna/donna-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Donna.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `DONNA_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `donna doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `donna.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
