...
read_when:
  - Running Donna behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Donna
...
- You run Donna behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Donna checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Donna extracts the user identity from the configured header
...
- Donna can stay on loopback HTTP behind the proxy.
...
If Donna itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://donna.example.com
    to: http://donna-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`donna security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Donna config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `donna security audit` and review findings
