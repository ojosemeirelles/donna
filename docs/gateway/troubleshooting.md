...
- `donna gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `donna doctor` reports no blocking config/service issues.
- `donna channels status --probe` shows connected/ready channels.
...
donna logs --follow
donna models status
donna config get agents.defaults.models
```
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `donna configure`). If you are running Donna via Podman using the dedicated `donna` user, the config lives at `~donna/.donna/donna.json`.
...
donna approvals get --node <idOrNameOrIp>
donna logs --follow
donna status
```
...
donna browser start --browser-profile donna
donna browser profiles
donna logs --follow
donna doctor
```
...
```bash
donna gateway status
donna config get gateway.mode
donna config get gateway.remote.url
donna config get gateway.auth.mode
```
...
```bash
donna config get gateway.bind
donna config get gateway.auth.token
donna gateway status
donna logs --follow
```
