...
- Publish as `donna/plugin-sdk` (or export from core under `donna/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `DonnaPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: DonnaConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: DonnaConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: DonnaConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: DonnaConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: DonnaConfig): string;
  };
};
```
...
- Introduce `donna/plugin-sdk`.
- Add `api.runtime` to `DonnaPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `donnaRuntime: ">=2026.2.0"`).
