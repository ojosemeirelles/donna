---
summary: "Use MiniMax M2.5 in Donna"
read_when:
  - You want MiniMax models in Donna
...
- **Compatibility:** Donna still accepts legacy `MiniMax-M2.5-Lightning` configs, but prefer
...
```bash
donna plugins enable minimax-portal-auth  # skip if already loaded.
donna gateway restart  # restart if gateway is already running
donna onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/donna/donna/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `donna.json`:
...
- Use `donna models list` and `donna models set minimax/MiniMax-M2.5` to switch.
...
- Running `donna configure` and selecting **MiniMax M2.5**, or
