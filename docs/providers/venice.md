---
summary: "Use Venice AI privacy-focused models in Donna"
read_when:
  - You want privacy-focused inference in Donna
...
```bash
donna onboard --auth-choice venice-api-key
```
...
```bash
donna agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, Donna shows all available Venice models. Pick based on your needs:
...
```bash
donna models set venice/claude-opus-45
donna models set venice/llama-3.3-70b
```

List all available models:

```bash
donna models list | grep venice
```

## Configure via `donna configure`

1. Run `donna configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Donna automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
donna agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
donna agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
donna agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
donna agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
donna agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `donna models list` to see currently available models. Some models may be temporarily offline.
