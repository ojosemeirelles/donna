---
summary: "Use OpenAI via API keys or Codex subscription in Donna"
read_when:
  - You want to use OpenAI models in Donna
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Donna.
...
```bash
donna onboard --auth-choice openai-api-key
# or non-interactive
donna onboard --openai-api-key "$OPENAI_API_KEY"
```
...
donna onboard --auth-choice openai-codex

# Or run OAuth directly
donna models auth login --provider openai-codex
```
...
Donna uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Donna also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Donna enables it by default for
...
`baseUrl` on `api.openai.com`), Donna now auto-enables OpenAI server-side
