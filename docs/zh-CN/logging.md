...
所有日志配置都在 `~/.donna/donna.json` 的 `logging` 下。
...
- Donna 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `donna plugins enable diagnostics-otel` 启用插件。
...
- `donna.tokens`（计数器，属性：`donna.token`、`donna.channel`、`donna.provider`、`donna.model`）
- `donna.cost.usd`（计数器，属性：`donna.channel`、`donna.provider`、`donna.model`）
- `donna.run.duration_ms`（直方图，属性：`donna.channel`、`donna.provider`、`donna.model`）
- `donna.context.tokens`（直方图，属性：`donna.context`、`donna.channel`、`donna.provider`、`donna.model`）

消息流：

- `donna.webhook.received`（计数器，属性：`donna.channel`、`donna.webhook`）
- `donna.webhook.error`（计数器，属性：`donna.channel`、`donna.webhook`）
- `donna.webhook.duration_ms`（直方图，属性：`donna.channel`、`donna.webhook`）
- `donna.message.queued`（计数器，属性：`donna.channel`、`donna.source`）
- `donna.message.processed`（计数器，属性：`donna.channel`、`donna.outcome`）
- `donna.message.duration_ms`（直方图，属性：`donna.channel`、`donna.outcome`）

队列 + 会话：

- `donna.queue.lane.enqueue`（计数器，属性：`donna.lane`）
- `donna.queue.lane.dequeue`（计数器，属性：`donna.lane`）
- `donna.queue.depth`（直方图，属性：`donna.lane` 或 `donna.channel=heartbeat`）
- `donna.queue.wait_ms`（直方图，属性：`donna.lane`）
- `donna.session.state`（计数器，属性：`donna.state`、`donna.reason`）
- `donna.session.stuck`（计数器，属性：`donna.state`）
- `donna.session.stuck_age_ms`（直方图，属性：`donna.state`）
- `donna.run.attempt`（计数器，属性：`donna.attempt`）
...
- `donna.webhook.error`
  - `donna.channel`、`donna.webhook`、`donna.chatId`、`donna.error`
- `donna.message.processed`
  - `donna.channel`、`donna.outcome`、`donna.chatId`、`donna.messageId`、`donna.sessionKey`、`donna.sessionId`、`donna.reason`
- `donna.session.stuck`
  - `donna.state`、`donna.ageMs`、`donna.queueDepth`、`donna.sessionKey`、`donna.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `donna doctor`。
