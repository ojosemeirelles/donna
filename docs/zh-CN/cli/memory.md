...
read_when:
  - 你想要索引或搜索语义记忆
  - 你正在调试记忆可用性或索引问题
summary: "`donna memory`（status/index/search）的 CLI 参考"
title: memory
x-i18n:
...
```bash
donna memory status
donna memory status --deep
donna memory status --deep --index
donna memory status --deep --index --verbose
donna memory index
donna memory index --verbose
donna memory search "release checklist"
