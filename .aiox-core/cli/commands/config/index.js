...
function splitL1(config) {
  ...
  if (config.lazyLoading) {
    l1.performance_defaults = { lazy_loading: config.lazyLoading };
  }
  if (config.git) {
  ...
  if (config.utils) {
    l1.utility_scripts_registry = config.utils;
  }

  // ide_sync_system (Section 12)
  if (config.ideSync) {
    l1.ide_sync_system = config.ideSync;
  }
...
function splitL2(config) {
  ...
  if (config.project) {
    l2.project = config.project;
  }

  // documentation_paths (Section 2)
  const docs = {};
  if (config.qa) {
    docs.qa_dir = config.qa.qaLocation;
  }
  if (config.prd) {
  ...
  if (config.devStoryLocation) {
    docs.stories_dir = config.devStoryLocation;
  }
  if (config.devDebugLog) {
    docs.dev_debug_log = config.devDebugLog;
  }
  if (config.slashPrefix) {
    docs.slash_prefix = config.slashPrefix;
  }
  if (config.customTechnicalDocuments !== undefined) {
    docs.custom_technical_documents = config.customTechnicalDocuments;
  }
  if (config.devLoadAlwaysFiles) {
    docs.dev_load_always_files = config.devLoadAlwaysFiles;
  }
  if (config.devLoadAlwaysFilesFallback) {
    docs.dev_load_always_files_fallback = config.devLoadAlwaysFilesFallback;
  }
  if (Object.keys(docs).length > 0) {
    l2.documentation_paths = docs;
  }

  // github_integration (Section 8)
  if (config.github) {
    l2.github_integration = config.github;
  }
  ...
  // pvMindContext
  if (config.pvMindContext) {
    l2.pv_mind_context = config.pvMindContext;
  }

  // storyBacklog
  if (config.storyBacklog) {
    l2.story_backlog = config.storyBacklog;
  }

  // auto_claude (Section 13)
  if (config.autoClaude) {
    l2.auto_claude = config.autoClaude;
  }
...
function splitL4(config) {
  ...
  if (config.coderabbit_integration) {
    const cr = config.coderabbit_integration;
    const l4cr = {};
    if (cr.installation_mode) {
      l4cr.installation_mode = cr.installation_mode;
    }
    if (cr.wsl_config) {
      l4cr.wsl_config = cr.wsl_config;
    }
    if (cr.commands) {
      l4cr.commands = cr.commands;
    }
    if (Object.keys(l4cr).length > 0) {
      l4.coderabbit_integration = l4cr;
    }
  }
...
function validateYamlSyntax(root, level, issues) {
  ...
  if (!fs.existsSync(fullPath)) {
    return;
  } // Optional files are OK to be missing
