...
class PRReviewAI extends EventEmitter {
  ...
  async fetchPRData(prNumber) {
    try {
      ...
      throw new Error(`Failed to fetch PR ${prNumber}: ${error.message}`, { cause: error });
    }
  }
  ...
  buildSummary(prData, findings, aiReview, verdict) {
    ...
    if (findings.length > 0) {
      ...
      if (bySeverity.high) {
        parts.push(`  - 🟠 High: ${bySeverity.high}`);
      }
      if (bySeverity.medium) {
        parts.push(`  - 🟡 Medium: ${bySeverity.medium}`);
      }
      if (bySeverity.low) {
        parts.push(`  - 🔵 Low: ${bySeverity.low}`);
      }
    }
