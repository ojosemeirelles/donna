...
class TestQualityAssessment {
  ...
  async analyzeSingleTestFile(testFilePath) {
    ...
    try {
      ...
      console.log(
        chalk.green(
          `✅ Test quality assessed: ${assessment.quality_rating} (${assessment.overall_score.toFixed(1)}/10)`,
        ),
      );
      ...
      console.error(
        chalk.red(`Failed to assess test quality for ${testFilePath}: ${error.message}`),
      );
      throw error;
    }
  }
  ...
  async assessTestSuite(testFiles, options = {}) {
    ...
    suiteAssessment.overall_suite_score =
    ...
    console.log(
      chalk.gray(`   Overall score: ${suiteAssessment.overall_suite_score.toFixed(1)}/10`),
    );
  ...
  analyzeTestOrganization(_content) {
    ...
    if (score.details.has_describe_blocks) {
      score.value += 2;
    }
    ...
    if (score.details.has_nested_structure) {
      score.value += 1;
    }
    ...
    if (score.details.has_setup_teardown) {
      score.value += 2;
    }
    ...
    score.details.tests_per_describe =
      describeBlocks.length > 0 ? testBlocks.length / describeBlocks.length : testBlocks.length;
    ...
    if (score.details.has_documentation) {
      score.value += 1;
    }
  ...
  analyzeNamingQuality(_content) {
    ...
    for (const name of testNames) {
      ...
      if (
        name.length > 20 &&
        (name.includes("should") || name.includes("when") || name.includes("given"))
      ) {
        descriptiveNames++;
      }
  ...
  analyzeAssertionQuality(_content) {
    ...
    const specificAssertions =
      assertions.specific.length + assertions.numeric.length + assertions.object.length;
    const genericAssertions = assertions.existence.length + assertions.boolean.length;
    score.details.specific_ratio =
      specificAssertions / (specificAssertions + genericAssertions + 1);
  ...
  analyzeTestCoverageBreadth(_content) {
    ...
    const scenarios = {
      ...
      edge_cases:
        content.match(/should\s+.*(?:empty|null|undefined|zero|negative|boundary|limit)/gi) || [],
    ...
    const scenariosCovered = Object.entries(scenarios).filter(
      ([_, matches]) => matches.length > 0,
    ).length;
    ...
    if (score.details.interaction_testing) {
      score.value += 3;
    }
  ...
  analyzeCodeClarity(_content) {
    ...
    score.details.descriptive_variables_ratio =
      variables.length > 0 ? descriptiveVariables / variables.length : 1;
  ...
  analyzeTestIsolation(_content) {
    ...
    if (score.details.has_isolation_setup) {
      score.value += 3;
    }
    ...
    if (score.details.shared_state_usage > 3) {
      score.value -= 2;
    } else if (score.details.shared_state_usage === 0) {
      score.value += 2;
    }
    ...
    // Mock cleanup
    const mockCleanup =
      content.match(/\b(?:jest\.clearAllMocks|sinon\.restore|vi\.clearAllMocks)\b/g) || [];
    score.details.mock_cleanup = mockCleanup.length > 0;
    if (score.details.mock_cleanup) {
      score.value += 3;
    }
  ...
  analyzeTestReliability(_content) {
    ...
    const totalFlakyPatterns = Object.values(flakyPatterns).reduce(
  ...
  analyzeMockQuality(_content) {
    ...
    const mockVerifications =
      content.match(/toHaveBeenCalled|toHaveBeenCalledWith|calledWith|called/g) || [];
  ...
  analyzeJestUsage(_content) {
    ...
    // Good Jest practices
    if (content.includes("expect.extend")) {
      score += 1;
    }
  ...
  identifyQualityIssues(metrics, content) {
    ...
    if (metrics.test_organization.value < 5) {
      issues.push({
        ...
        suggestion:
          "Use describe blocks to group related tests and add beforeEach/afterEach for setup",
      });
    }
  ...
  generateSuiteRecommendations(suiteAssessment) {
    ...
    const poorQualityFiles =
      suiteAssessment.quality_distribution.poor + suiteAssessment.quality_distribution.very_poor;
  ...
  getQualityTrends() {
    if (this.qualityHistory.length < 2) {
      return {
        ...
        message: "Need at least 2 assessments for trend calculation",
      };
    }
