...
function processTaskFile(filename) {
  ...
  if (
    content.includes("**Strategy:** {TODO: Fail-fast | Graceful degradation | Retry with backoff}")
  ) {
    content = content.replace(
...
function main() {
  ...
  files.forEach((filename) => {
    try {
      ...
      if (result.processed) {
        ...
        console.log(
          `   └─ ${result.atomicLayer} | ${result.duration} | ${result.cost} | ${result.errorStrategy}`,
        );
  ...
  console.log(
    `   TOTAL: ${results.stats.durationsSet + results.stats.costsSet + results.stats.strategiesSet}`,
  );
