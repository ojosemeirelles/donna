...
function analyzeDirectoryStructure(targetDir, analysis) {
  ...
  const srcDirs = new Set(["src", "lib", "app", "source", "pkg", "cmd", "internal"]);
  const testDirs = new Set(["test", "tests", "__tests__", "spec"]);
  const docDirs = new Set(["docs", "doc", "documentation"]);
  ...
  try {
    ...
    for (const item of contents) {
      ...
      if (fs.statSync(itemPath).isDirectory()) {
        if (srcDirs.has(item.toLowerCase())) {
          hasSrcDir = true;
        }
        if (testDirs.has(item.toLowerCase())) {
          hasTestDir = true;
        }
        if (docDirs.has(item.toLowerCase())) {
          hasDocDir = true;
        }
      }
    }
