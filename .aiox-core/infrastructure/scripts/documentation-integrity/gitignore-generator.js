...
const GitignoreTemplates = {
  AIOX_BASE: "gitignore-aiox-base.tmpl",
...
function detectTechStacks(markers) {
  ...
  if (markers.hasPackageJson) {
    stacks.push(TechStack.NODE);
  }
  if (markers.hasPythonProject) {
    stacks.push(TechStack.PYTHON);
  }
  if (markers.hasGoMod) {
    stacks.push(TechStack.GO);
  }
  if (markers.hasCargoToml) {
    stacks.push(TechStack.RUST);
  }
...
function getTemplatesForStacks(techStacks) {
  const templates = [GitignoreTemplates.AIOX_BASE];
...
function generateGitignore(markers, options = {}) {
  ...
  sections.push("# Generated by AIOX Documentation Integrity System");
...
/**
 * Merges AIOX ignores with existing .gitignore
 *
...
function mergeGitignore(existingContent, _options = {}) {
  // Check if AIOX section already exists
  if (existingContent.includes("AIOX Integration Section")) {
    console.log("AIOX section already exists in .gitignore, skipping merge");
...
# AIOX Local Configuration
.aiox-core/local/
.aiox-core/*.local.yaml
.aiox-core/logs/
.aiox-core/cache/
...
/**
 * Checks if a .gitignore file has AIOX integration
 *
 * @param {string} targetDir - Target directory
 * @returns {boolean} True if AIOX section exists
 */
function hasAioxIntegration(targetDir) {
  const gitignorePath = path.join(targetDir, ".gitignore");
  ...
  return content.includes("AIOX Integration Section") || content.includes(".aiox-core/");
}
...
function parseGitignore(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes("AIOX Integration Section")) {
      if (aioxStart === -1) {
        aioxStart = i;
      }
    }

    if (line.includes("End of AIOX Integration Section")) {
      aioxEnd = i;
    }

    if (line.startsWith("#") && !line.includes("AIOX")) {
  ...
  if (aioxStart !== -1) {
    sections.aioxSection = {
      start: aioxStart,
      end: aioxEnd !== -1 ? aioxEnd : lines.length - 1,
    };
  }
