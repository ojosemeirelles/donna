...
class ComponentSearch {
  ...
  async findComponent(componentType, componentName) {
    ...
    try {
      ...
      console.warn(
        chalk.yellow(
          `Failed to find component ${componentType}/${componentName}: ${error.message}`,
        ),
      );
      return null;
    }
    ...
    try {
      ...
      if (matchingFiles.length > 0) {
        ...
        const bestMatch = matchingFiles.reduce((best, current) => {
          ...
          const currentScore = this.calculateSimilarity(
            componentName,
            path.basename(current, extension),
          );
  ...
  async findSimilarComponents(componentType, componentName, limit = 5) {
    ...
    return suggestions.toSorted((a, b) => b.similarity - a.similarity).slice(0, limit);
  }
