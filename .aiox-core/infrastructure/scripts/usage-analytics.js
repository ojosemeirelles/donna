...
class UsageAnalytics {
  ...
  calculateUsageFrequency(usage) {
    if (usage.direct_references === 0) {
      return "never";
    }
    if (usage.direct_references < 3) {
      return "rare";
    }
    if (usage.direct_references < 10) {
      return "occasional";
    }
    if (usage.direct_references < 25) {
      return "frequent";
    }
  ...
  identifyHotspots(componentUsage) {
    return Object.entries(componentUsage)
      ...
      .toSorted((a, b) => b.popularity_score - a.popularity_score);
  }
  ...
  identifyUsagePattern(content, component) {
    ...
    if (content.includes("*" + component.id)) {
      return "task_call";
    }
