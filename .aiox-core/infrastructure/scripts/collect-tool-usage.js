...
function pruneOldEntries(sessions) {
  ...
  const pruned = sessions.filter((session) => {
    ...
    if (session.timestamp && session.timestamp >= cutoffISO) {
      return true;
    }
...
async function main() {
  ...
  if (pruneOnly) {
    ...
    if (jsonOutput) {
      ...
      if (dryRun) {
        console.log("[TOK-5] Dry run — no changes written.");
      }
    }
    return;
  }
  ...
  if (jsonOutput) {
    ...
    if (removed > 0) {
      console.log(`[TOK-5] Pruned ${removed} old sessions.`);
    }
    if (dryRun) {
      console.log("[TOK-5] Dry run — no changes written.");
    } else {
      console.log(`[TOK-5] Data saved to ${USAGE_FILE}`);
    }
  }
}
