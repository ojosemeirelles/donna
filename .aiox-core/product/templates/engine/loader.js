...
function parseTemplate(content) {
  ...
  try {
    ...
    throw new Error(`Failed to parse template frontmatter: ${error.message}`, { cause: error });
  }
  ...
  async load(templateType, options = {}) {
    ...
    try {
      ...
      if (error.code === "ENOENT") {
        throw new Error(`Template not found: ${templateType} (${templatePath})`, { cause: error });
      }
