  ...
  addComponentToManifest(manifest, componentType, componentInfo) {
    ...
    switch (componentType) {
      case "agent":
        ...
        if (!updated.team.agents) {
          updated.team.agents = [];
        }

        // Check if agent already exists
        const existingAgentIndex = updated.team.agents.findIndex((a) => a.id === componentInfo.id);
      ...
      case "workflow":
        ...
        const existingWorkflowIndex = updated.workflows.findIndex((w) => w.id === componentInfo.id);
