...
class ConflictResolver {
  ...
  detectConflictType(file, conflictInfo) {
    ...
    for (const _conflict of conflicts) {
      ...
      if (
        (oursContent.includes("import") || oursContent.includes("require")) &&
  ...
  async resolveManual(_fileInfo) {
    ...
    console.log(chalk.yellow(`Manual resolution required for ${fileInfo.file}`));
    console.log(chalk.gray("Edit the file to resolve conflicts, then mark as resolved"));
  ...
  async resolveInteractive(_fileInfo) {
    ...
    for (let i = 0; i < conflicts.length; i++) {
      ...
      if (conflict.ours.length > 5) {
        console.log(chalk.gray("..."));
      }

      console.log(chalk.green("\n>>>> THEIRS:"));
      console.log(conflict.theirs.slice(0, 5).join("\n"));
      if (conflict.theirs.length > 5) {
        console.log(chalk.gray("..."));
      }
      ...
      if (resolution !== "skip") {
        resolvedContent = await this.applyResolution(resolvedContent, _conflict, resolution);
        resolvedCount++;
      }
    }
