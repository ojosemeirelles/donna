...
class VersionTracker {
  ...
  async recordVersion(versionData) {
    try {
      ...
      if (this.currentVersion && !semver.gt(version, this.currentVersion)) {
        throw new Error(
          `New version ${version} must be greater than current ${this.currentVersion}`,
        );
      }
      ...
      if (migration_required) {
        console.log(
          chalk.yellow(
            `⚠️  Migration required from ${this.versionInfo.previous_version} to ${version}`,
          ),
        );
      }
  ...
  async deprecateVersion(version, deprecationInfo = {}) {
    try {
      ...
      const deprecation = {
        ...
        deprecation_notice:
          deprecationInfo.notice ||
          `Version ${version} is deprecated. Please upgrade to ${this.currentVersion}.`,
      };
  ...
  async getMigrationPath(fromVersion, toVersion) {
    ...
    try {
      ...
      path.requirements = [...new Set(path.steps.flatMap((step) => step.prerequisites))];
  ...
  async loadVersionInfo() {
    try {
      ...
      throw new Error(`Failed to load version info: ${error.message}`, { cause: _error });
    }
  }
  ...
  async saveVersionInfo() {
    try {
      ...
      throw new Error(`Failed to save version info: ${error.message}`, { cause: _error });
    }
  }
  ...
  getVersionsInPath(fromVersion, toVersion) {
    ...
    return versions.toSorted((a, b) => semver.compare(a.version, b.version));
  }
  ...
  assessMigrationRisk(version) {
    ...
    if (breakingChanges > 5 || componentsModified > 20) {
      return "high";
    }
    if (breakingChanges > 2 || componentsModified > 10) {
      return "medium";
    }
    return "low";
  }
