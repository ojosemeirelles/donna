...
function isNoise(value) {
  ...
  if (trimmed.length <= 2 && !KNOWN_AGENTS.includes(trimmed)) {
    return true;
  }
...
function extractMarkdownCrossReferences(content, entityId, verbose = false) {
  ...
  const addDep = (ref) => {
    ...
    if (isNoise(ref)) {
      if (verbose) {
        console.log(`[IDS] Filtered noise "${ref}" from MD cross-refs in "${entityId}"`);
      }
      return;
    }
    deps.add(ref);
...
function scanCategory(config, verbose = false) {
  ...
  for (const filePath of files) {
    ...
    if (frontmatterMatch) {
      ...
      if (lfMatch) {
        lifecycleOverride = lfMatch[1].trim();
      }
    }
    ...
    if (!lifecycleOverride) {
      ...
      if (inlineMatch) {
        lifecycleOverride = inlineMatch[1].trim();
      }
    }
