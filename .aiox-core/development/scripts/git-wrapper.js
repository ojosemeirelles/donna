...
/**
 * Git operations wrapper for AIOX framework modifications
 */
class GitWrapper {
  constructor(options = {}) {
  ...
  async execGit(command, options = {}) {
    try {
      ...
      throw new Error(`Git command failed: ${error.message}`, { cause: _error });
    }
  }
  ...
  async commit(message, options = {}) {
    const { author = "aiox-developer <aiox-developer@aiox-fullstack.local>", signoff = true } =
      options;
  ...
  async mergeBranch(branchName, options = {}) {
    const { strategy = "recursive", message = null, noFastForward = true } = options;
  ...
  async getDiff(options = {}) {
    const { from = "HEAD", to = null, files = [], nameOnly = false } = options;
