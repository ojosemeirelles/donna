...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked DONNA_HOME.
  delete process.env.DONNA_HOME;
  process.env.DONNA_STATE_DIR = path.join(base, ".donna");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "donna-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".donna", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
