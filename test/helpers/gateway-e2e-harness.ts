...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `donna-e2e-${name}-`));
  const configDir = path.join(homeDir, ".donna");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          DONNA_SKIP_PROVIDERS: "1",
          DONNA_SKIP_GMAIL_WATCHER: "1",
          DONNA_SKIP_CRON: "1",
          DONNA_SKIP_BROWSER_CONTROL_SERVER: "1",
          DONNA_SKIP_CANVAS_HOST: "1",
          DONNA_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
