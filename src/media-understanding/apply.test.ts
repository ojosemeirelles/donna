...
const TEMP_MEDIA_PREFIX = "donna-media-";
...
function createGroqAudioConfig(): DonnaConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): DonnaConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): DonnaConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: DonnaConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredDonnaTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        DONNA_AGENT_DIR: isolatedAgentDir,
