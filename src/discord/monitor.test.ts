...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of Donna")).toBe("friends-of-donna");
  ...
  it("matches ids by default and names only when enabled", () => {
    ...
    expect(allowListMatches(allow, { name: "friends-of-donna" })).toBe(false);
    expect(allowListMatches(allow, { name: "steipete" }, { allowNameMatching: true })).toBe(true);
    expect(allowListMatches(allow, { name: "friends-of-donna" }, { allowNameMatching: true })).toBe(
      true,
    );
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-donna" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Donna"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-donna");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-donna": { slug: "friends-of-donna" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Donna"),
      guildEntries,
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["donna-dm"],
        channelId: "1",
        channelName: "Donna DM",
        channelSlug: "donna-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
