...
let ensureDonnaCliOnPath: typeof import("./path-env.js").ensureDonnaCliOnPath;

describe("ensureDonnaCliOnPath", () => {
  const envKeys = [
    "PATH",
    "DONNA_PATH_BOOTSTRAPPED",
    "DONNA_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureDonnaCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling donna exists", () => {
    const tmp = abs("/tmp/donna-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "donna");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.DONNA_PATH_BOOTSTRAPPED;

    ensureDonnaCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.DONNA_PATH_BOOTSTRAPPED = "1";
    ensureDonnaCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/donna-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "donna");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/donna-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "donna");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "donna");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/donna-path/case-linuxbrew");
