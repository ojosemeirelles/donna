...
const DEFAULT_PACKAGE_NAME = "donna";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["donna", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "donna-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "donna.mjs");
    ...
    const doctorStep = await runStep(
      step("donna doctor", doctorArgv, gitRoot, { DONNA_UPDATE_IN_PROGRESS: "1" }),
    );
