...
import { POSIX_DONNA_TMP_DIR, resolvePreferredDonnaTmpDir } from "./tmp-donna-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredDonnaTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `donna-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredDonnaTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_DONNA_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredDonnaTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredDonnaTmpDir", () => {
  it("prefers /tmp/donna when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_DONNA_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/donna when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_DONNA_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/donna when /tmp/donna is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/donna when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/donna is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/donna is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/donna is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Donna temp dir/);
  });
