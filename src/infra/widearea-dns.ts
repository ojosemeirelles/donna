...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.DONNA_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*donna-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "donna";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "donna");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "donna-gw");
  ...
  records.push(`_donna-gw._tcp IN PTR ${instanceLabel}._donna-gw._tcp`);
  records.push(`${instanceLabel}._donna-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._donna-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; donna-content-hash: ${contentHash}\n${contentBody}`;
}
