...
vi.mock("./donna-root.js", () => ({
  resolveDonnaPackageRoot: vi.fn(async () => null),
  resolveDonnaPackageRootSync: vi.fn(() => null),
}));
...
let donnaRoot: typeof import("./donna-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    donnaRoot = await import("./donna-root.js");
  });
  ...
  it("uses resolveDonnaPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/donna");
    (
      donnaRoot.resolveDonnaPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/donna"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "donna" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "donna.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-donna");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/donna-bundle");
    (
      donnaRoot.resolveDonnaPackageRootSync as unknown as ReturnType<typeof vi.fn>
