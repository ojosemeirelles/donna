...
export function triggerDonnaRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(process.env.DONNA_SYSTEMD_UNIT, process.env.DONNA_PROFILE);
  ...
  const label =
    process.env.DONNA_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.DONNA_PROFILE);
