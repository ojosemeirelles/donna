...
import { triggerDonnaRestart } from "./restart.js";
 ...
 * - DONNA_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
 * - otherwise: spawn detached child with current argv/execArgv, then caller exits
 */
export function restartGatewayProcessWithFreshPid(): GatewayRespawnResult {
  if (isTruthy(process.env.DONNA_NO_RESPAWN)) {
  ...
  if (isLikelySupervisedProcess(process.env)) {
    ...
    if (process.platform === "darwin" && process.env.DONNA_LAUNCHD_LABEL?.trim()) {
      const restart = triggerDonnaRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
