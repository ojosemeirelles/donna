...
function candidateBinDirs(opts: EnsureDonnaPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `donna` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "donna");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.DONNA_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "donna"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `donna` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureDonnaCliOnPath(opts: EnsureDonnaPathOpts = {}) {
  if (isTruthyEnvValue(process.env.DONNA_PATH_BOOTSTRAPPED)) {
    return;
  }
