...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses donna-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "donna-gateway" },
          { pid: process.pid, cmd: "donna-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'donna'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple donna pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "donna-gateway" },
          { pid: pid2, cmd: "donna-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-donna pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks donna processes — non-donna listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 1) {
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: stalePid, cmd: "donna-gateway" }]),
            stderr: "",
          };
        }
    ...
    it("lsof status 1 with non-empty donna stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty donna stdout must not be treated as port-free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        if (call === 2) {
          // status 1 + donna pid in stdout — container-restricted lsof reports partial results
          return {
            error: null,
            status: 1,
            stdout: lsofOutput([{ pid: stalePid, cmd: "donna-gateway" }]),
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'donna'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("donna")
      // inside the mid-loop flush: a non-donna cmd between two entries must not
      // be pushed, but the following donna entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-donna entry first, then donna entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid donna entry must still be found.
      const stdout = `p0\ncdonna-gateway\np${stalePid}\ncdonna-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero donna pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-donna stdout (line 145)", () => {
    it("treats status 1 + non-donna stdout as port-free (not an donna process)", () => {
      // status 1 + non-empty stdout where no donna pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill donna pids) it is effectively free.
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        // status 1 + non-donna output — should be treated as free:true for our purposes
        return {
          error: null,
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no donna pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
