...
const mocks = vi.hoisted(() => ({
  ...
  loadDonnaPlugins: vi.fn(),
}));

const TEST_WORKSPACE_ROOT = "/tmp/donna-test-workspace";
...
vi.mock("../../plugins/loader.js", () => ({
  loadDonnaPlugins: mocks.loadDonnaPlugins,
}));
...
describe("resolveOutboundTarget channel resolution", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadDonnaPlugins.mockReset();
  });
  ...
  it("recovers telegram plugin resolution so announce delivery does not fail with Unsupported channel: telegram", () => {
    ...
    expect(mocks.loadDonnaPlugins).toHaveBeenCalledTimes(1);
  });
  ...
  it("retries bootstrap on subsequent resolve when the first bootstrap attempt fails", () => {
    ...
    expect(mocks.loadDonnaPlugins).toHaveBeenCalledTimes(2);
  });
});
