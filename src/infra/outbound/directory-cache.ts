...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: DonnaConfig | null = null;
  ...
  get(key: string, cfg: DonnaConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: DonnaConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: DonnaConfig): void {
