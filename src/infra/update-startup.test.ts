...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "donna-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["DONNA_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.DONNA_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveDonnaPackageRoot } = await import("./donna-root.js"));
    ...
    vi.mocked(resolveDonnaPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveDonnaPackageRoot).mockResolvedValue("/opt/donna");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/donna/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
