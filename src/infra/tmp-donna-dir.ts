...
export const POSIX_DONNA_TMP_DIR = "/tmp/donna";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredDonnaTmpDirOptions = {
...
export function resolvePreferredDonnaTmpDir(
  options: ResolvePreferredDonnaTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "donna" : `donna-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[donna] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Donna temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Donna temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_DONNA_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_DONNA_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_DONNA_TMP_DIR)) {
      return POSIX_DONNA_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_DONNA_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_DONNA_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_DONNA_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_DONNA_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_DONNA_TMP_DIR;
