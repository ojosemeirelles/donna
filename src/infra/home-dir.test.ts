...
describe("resolveEffectiveHomeDir", () => {
  it("prefers DONNA_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(path.resolve("/srv/donna-home"));
  });
  ...
  it("expands DONNA_HOME when set to ~", () => {
    const env = {
      DONNA_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for DONNA_HOME", () => {
    const result = resolveRequiredHomeDir(
      { DONNA_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when DONNA_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ DONNA_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { DONNA_HOME: "/srv/donna-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/donna-home")}/x`);
  });
