...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.DONNA_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Donna";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Donna\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.DONNA_MDNS_HOSTNAME?.trim() || process.env.DONNA_MDNS_HOSTNAME?.trim() || "donna";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
