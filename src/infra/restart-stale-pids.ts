 ...
 * returning control to the caller (typically `triggerDonnaRestart` →
...
/**
 * Parse donna gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("donna")) {
 ...
 * Returns only PIDs that belong to donna gateway processes (not the current process).
 */
