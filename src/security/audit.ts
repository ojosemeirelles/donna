...
import type { ConfigFileSnapshot, DonnaConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: DonnaConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Donna state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Donna state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.DONNA_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.DONNA_GATEWAY_PASSWORD) || hasNonEmptyString(env.DONNA_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("donna security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.DONNA_GATEWAY_TOKEN) ||
    hasNonEmptyString(env.DONNA_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(env.DONNA_GATEWAY_PASSWORD) ||
        hasNonEmptyString(env.DONNA_GATEWAY_PASSWORD) ||
...
function collectLoggingFindings(cfg: DonnaConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: DonnaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: DonnaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("donna status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("donna security audit --deep")}".`,
    });
  }
