...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "donna.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "donna.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.DONNA_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.DONNA_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.DONNA_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.DONNA_NODE_OPTIONS_READY = "1";
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    import("./version.js")
      ...
      .catch((error) => {
        console.error(
          "[donna] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[donna] ${parsed.error}`);
      process.exit(2);
    }
