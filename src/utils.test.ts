...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("donna-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("donna-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("donna-lid-a-", (first) => {
      withTempDirSync("donna-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.donna when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "donna-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers DONNA_HOME over HOME", () => {
    vi.stubEnv("DONNA_HOME", "/srv/donna-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/donna-home"));
...
describe("shortenHomePath", () => {
  it("uses $DONNA_HOME prefix when DONNA_HOME is set", () => {
    vi.stubEnv("DONNA_HOME", "/srv/donna-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/donna-home")}/.donna/donna.json`)).toBe(
      "$DONNA_HOME/.donna/donna.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $DONNA_HOME replacement when DONNA_HOME is set", () => {
    vi.stubEnv("DONNA_HOME", "/srv/donna-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/donna-home")}/.donna/donna.json`),
    ).toBe("config: $DONNA_HOME/.donna/donna.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/donna")).toBe(path.resolve(os.homedir(), "donna"));
  });
  ...
  it("prefers DONNA_HOME for tilde expansion", () => {
    vi.stubEnv("DONNA_HOME", "/srv/donna-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/donna")).toBe(path.resolve("/srv/donna-home", "donna"));
