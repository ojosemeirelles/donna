...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.DONNA_GATEWAY_TOKEN?.trim();
  if (serviceToken === expectedToken) {
    return;
  }
  issues.push({
    code: SERVICE_AUDIT_CODES.gatewayTokenMismatch,
    message: "Gateway service DONNA_GATEWAY_TOKEN does not match gateway.auth.token in donna.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `donna gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
