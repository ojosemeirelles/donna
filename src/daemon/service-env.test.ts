...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.DONNA_GATEWAY_PORT).toBe("18789");
    expect(env.DONNA_GATEWAY_TOKEN).toBe("secret");
    expect(env.DONNA_SERVICE_MARKER).toBe("donna");
    expect(env.DONNA_SERVICE_KIND).toBe("gateway");
    expect(typeof env.DONNA_SERVICE_VERSION).toBe("string");
    expect(env.DONNA_SYSTEMD_UNIT).toBe("donna-gateway.service");
    if (process.platform === "darwin") {
      expect(env.DONNA_LAUNCHD_LABEL).toBe("ai.donna.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", DONNA_PROFILE: "work" },
      port: 18789,
    });
    expect(env.DONNA_SYSTEMD_UNIT).toBe("donna-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.DONNA_LAUNCHD_LABEL).toBe("ai.donna.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through DONNA_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", DONNA_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.DONNA_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits DONNA_GATEWAY_TOKEN when token env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        DONNA_GATEWAY_TOKEN: "   ",
    ...
    expect(env.DONNA_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".donna"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", DONNA_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".donna-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", DONNA_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".donna"));
  });

  it("uses DONNA_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", DONNA_STATE_DIR: "/var/lib/donna" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/donna"));
  });

  it("expands ~ in DONNA_STATE_DIR", () => {
    const env = { HOME: "/Users/test", DONNA_STATE_DIR: "~/donna-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/donna-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { DONNA_STATE_DIR: "C:\\State\\donna" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\donna");
  });
});
