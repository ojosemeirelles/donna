...
declare const __DONNA_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "donna";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["DONNA_VERSION"],
      runtimeVersion,
      env["DONNA_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Donna version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __DONNA_VERSION__ === "string" ? __DONNA_VERSION__ : undefined,
  bundledVersion: process.env.DONNA_BUNDLED_VERSION,
});
