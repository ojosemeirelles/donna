...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["DONNA_GATEWAY_TOKEN", "DONNA_GATEWAY_PASSWORD"]);
  ...
  it("uses DONNA_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ DONNA_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers DONNA_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ DONNA_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
