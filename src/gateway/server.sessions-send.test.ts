...
const { createDonnaTools } = await import("../agents/donna-tools.js");
...
type SessionSendTool = ReturnType<typeof createDonnaTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createDonnaTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["DONNA_GATEWAY_PORT", "DONNA_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.DONNA_GATEWAY_PORT = String(gatewayPort);
  process.env.DONNA_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("DONNA_CONFIG_PATH missing in gateway test environment");
      }
