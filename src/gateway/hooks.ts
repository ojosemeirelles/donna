...
export function resolveHooksConfig(cfg: DonnaConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: DonnaConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-donna-token"] === "string" ? req.headers["x-donna-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
