...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("donna doctor --fix");
...
describe("GatewayClient security checks", () => {
  const envSnapshot = captureEnv(["DONNA_ALLOW_INSECURE_PRIVATE_WS"]);
  ...
  it("allows ws:// to private addresses only with DONNA_ALLOW_INSECURE_PRIVATE_WS=1", () => {
