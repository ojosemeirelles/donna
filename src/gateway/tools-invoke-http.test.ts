...
let lastCreateDonnaToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/donna-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateDonnaToolsContext?.agentTo,
          agentThreadId: lastCreateDonnaToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createDonnaTools: (ctx: Record<string, unknown>) => {
      lastCreateDonnaToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateDonnaToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateDonnaToolsContext?.allowMediaInvokeCommands).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-donna-message-to": "channel:24514",
        "x-donna-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
