...
function cfg(input: Partial<DonnaConfig>): DonnaConfig {
  return input as DonnaConfig;
}

type ResolveFromConfigInput = Parameters<typeof resolveGatewayCredentialsFromConfig>[0];
type GatewayConfig = NonNullable<DonnaConfig["gateway"]>;
...
const DEFAULT_GATEWAY_ENV = {
  DONNA_GATEWAY_TOKEN: "env-token",
  DONNA_GATEWAY_PASSWORD: "env-password",
...
describe("resolveGatewayCredentialsFromConfig", () => {
  ...
  it("can disable legacy DONNA env fallback", () => {
    const resolved = resolveGatewayCredentialsFromConfig({
      cfg: cfg({
        gateway: {
          mode: "local",
      ...
      env: {
        DONNA_GATEWAY_TOKEN: "legacy-token",
        DONNA_GATEWAY_PASSWORD: "legacy-password",
      } as NodeJS.ProcessEnv,
      includeLegacyEnv: false,
    });
