...
describe("shared/frontmatter", () => {
  ...
  test("resolveDonnaManifestBlock parses JSON5 metadata and picks donna block", () => {
    const frontmatter = {
      metadata: "{ donna: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveDonnaManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveDonnaManifestBlock returns undefined for invalid input", () => {
    expect(resolveDonnaManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(resolveDonnaManifestBlock({ frontmatter: { metadata: "not-json5" } })).toBeUndefined();
    expect(
      resolveDonnaManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
