...
export async function finalizeOnboardingWizard(
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.donna/donna.json (gateway.auth.token) or DONNA_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("donna config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("donna doctor --generate-gateway-token")}`,
        "Web UI stores a copy in this browser's localStorage (donna.control.settings.v1).",
        `Open the dashboard anytime: ${formatCliCommand("donna dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control Donna."
            : "Copy/paste this URL in a browser on this machine to control Donna.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("donna dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    ["Back up your agent workspace.", "Docs: https://docs.donna.ai/concepts/agent-workspace"].join(
      "\n",
    ),
    "Workspace backup",
  );
  ...
  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.donna.ai/security",
    "Security",
  );
  ...
  await prompter.note(
    hasWebSearchKey
      ...
      : [
          ...
          `- Run: ${formatCliCommand("donna configure --section web")}`,
  ...
  await prompter.note(
    'What now: https://donna.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control Donna."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control Donna.",
  );
