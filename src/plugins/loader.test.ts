...
import { __testing, loadDonnaPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "donna-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.DONNA_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "donna.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.DONNA_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadDonnaPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          donna: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.DONNA_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.DONNA_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadDonnaPlugins>) {
...
function useNoBundledPlugins() {
  process.env.DONNA_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadDonnaPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.DONNA_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.DONNA_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadDonnaPlugins", () => {
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadDonnaPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadDonnaPlugins(options);
  ...
  it("skips importing bundled memory plugins that are disabled by memory slot", () => {
    ...
    fs.writeFileSync(
      path.join(memoryADir, "donna.plugin.json"),
      JSON.stringify(
        {
          id: "memory-a",
          kind: "memory",
    ...
    fs.writeFileSync(
      path.join(memoryBDir, "donna.plugin.json"),
      JSON.stringify(
        {
          id: "memory-b",
          kind: "memory",
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ DONNA_STATE_DIR: stateDir }, () => {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("donna/plugin-sdk").emptyPluginConfigSchema)(),
  register() {},
};`,
    });
