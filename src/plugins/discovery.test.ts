...
import { clearPluginDiscoveryCache, discoverDonnaPlugins } from "./discovery.js";

const tempDirs: string[] = [];

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `donna-plugins-${randomUUID()}`);
...
async function withStateDir<T>(stateDir: string, fn: () => Promise<T>) {
  return await withEnvAsync(
    {
      DONNA_STATE_DIR: stateDir,
      DONNA_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
    },
    fn,
  );
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverDonnaPlugins>[0],
) {
  return await withStateDir(stateDir, async () => {
    return discoverDonnaPlugins(params);
  });
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      donna: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverDonnaPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".donna", "extensions");
    fs.mkdirSync(workspaceExt, { recursive: true });
    fs.writeFileSync(path.join(workspaceExt, "beta.ts"), "export default function () {}", "utf-8");

    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverDonnaPlugins({ workspaceDir });
    });
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@donna/demo-plugin-dir",
      extensions: ["./index.js"],
    });
    fs.writeFileSync(path.join(packDir, "index.js"), "module.exports = {}", "utf-8");

    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverDonnaPlugins({ extraPaths: [packDir] });
    });
  ...
  it.runIf(process.platform !== "win32" && typeof process.getuid === "function")(
    ...
    async () => {
      ...
      const result = await withStateDir(stateDir, async () => {
        return discoverDonnaPlugins({ ownershipUid: actualUid + 1 });
      });
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = await withEnvAsync(
      {
        DONNA_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
      async () => withStateDir(stateDir, async () => discoverDonnaPlugins({})),
    );
