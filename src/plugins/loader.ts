...
import { discoverDonnaPlugins } from "./discovery.js";
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const entry of pluginSdkScopedAliasEntries) {
    ...
    if (resolved) {
      aliasMap[`donna/plugin-sdk/${entry.subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: DonnaPluginDefinition;
  register?: DonnaPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as DonnaPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as DonnaPluginDefinition;
...
export function loadDonnaPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverDonnaPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "donna/plugin-sdk": pluginSdkAlias } : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: DonnaPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as DonnaPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
