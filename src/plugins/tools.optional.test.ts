...
const loadDonnaPluginsMock = vi.fn();

vi.mock("./loader.js", () => ({
  loadDonnaPlugins: (params: unknown) => loadDonnaPluginsMock(params),
}));
...
function setRegistry(entries: MockRegistryToolEntry[]) {
  ...
  loadDonnaPluginsMock.mockReturnValue(registry);
  return registry;
}
...
describe("resolvePluginTools optional tools", () => {
  beforeEach(() => {
    loadDonnaPluginsMock.mockClear();
  });
