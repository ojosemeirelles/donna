...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing donna.extensions; update the plugin package to include donna.extensions (for example ["./dist/index.js"]). See https://docs.donna.ai/help/troubleshooting#plugin-install-fails-with-missing-donna-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_DONNA_EXTENSIONS: "missing_donna_extensions",
  EMPTY_DONNA_EXTENSIONS: "empty_donna_extensions",
...
function ensureDonnaExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_DONNA_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json donna.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_DONNA_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureDonnaExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from donna.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-donna"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "donna security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "donna security audit --deep" after install.`,
    );
  }
