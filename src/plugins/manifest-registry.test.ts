...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `donna-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "donna.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "donna.plugin.json");
  const linkedManifest = path.join(rootDir, "donna.plugin.json");
