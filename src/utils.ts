...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".donna");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.DONNA_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$DONNA_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via DONNA_STATE_DIR.
