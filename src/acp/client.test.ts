...
const createTempDir = () => tempDirs.make("donna-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets DONNA_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "donna",
    });

    expect(env.DONNA_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("donna");
  });

  it("overrides pre-existing DONNA_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      DONNA_SHELL: "wrong",
    });
    expect(env.DONNA_SHELL).toBe("acp-client");
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "donna", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "donna", "dist", "entry.js");
    const shimPath = path.join(dir, "donna.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\donna\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/donna-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/donna-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/donna-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
