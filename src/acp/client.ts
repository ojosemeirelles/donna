...
export function resolveAcpClientSpawnEnv(
  ...
  return { ...baseEnv, DONNA_SHELL: "acp-client" };
}
...
export async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  const serverCommand = opts.serverCommand ?? (entryPath ? process.execPath : "donna");
  ...
  await client.initialize({
    ...
    clientInfo: { name: "donna-acp-client", version: "1.0.0" },
  });
