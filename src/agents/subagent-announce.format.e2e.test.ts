...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/donna/donna/issues/31298
    previousFastTestEnv = process.env.DONNA_TEST_FAST;
  ...
  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.DONNA_TEST_FAST;
      return;
    }
    process.env.DONNA_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // DONNA_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Donna runtime context (internal):");
