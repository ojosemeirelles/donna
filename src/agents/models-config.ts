...
type ModelsConfig = NonNullable<DonnaConfig["models"]>;
...
export async function ensureDonnaModelsJson(
  config?: DonnaConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveDonnaAgentDir();
