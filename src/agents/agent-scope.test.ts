...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: DonnaConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: DonnaConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: DonnaConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: DonnaConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: DonnaConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: DonnaConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/donna");
  });

  it("uses DONNA_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "donna-home");
    vi.stubEnv("DONNA_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as DonnaConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".donna", "workspace"));
  });

  it("uses DONNA_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "donna-home");
    vi.stubEnv("DONNA_HOME", home);
    // Clear state dir so it falls back to DONNA_HOME
    vi.stubEnv("DONNA_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as DonnaConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".donna", "agents", "main", "agent"));
  });
});
