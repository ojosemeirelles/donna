...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["DONNA_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "donna-subagent-"));
  ...
  it("uses isolated temp state when DONNA_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "donna-test-state"));
  });
});
