...
const GOOGLE_LIVE_SINGLE_KEY = "DONNA_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "DONNA_LIVE_ANTHROPIC_KEY",
    listVar: "DONNA_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "DONNA_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `DONNA_LIVE_${base}_KEY`;
