...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "donna.sandbox=1");
  args.push("--label", `donna.sessionKey=${params.scopeKey}`);
  args.push("--label", `donna.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `donna.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "donna.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`donna sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`donna sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("donna sandbox recreate --all");
}
