...
describe("resolveApiKeyForProfile fallback to main agent", () => {
  const envSnapshot = captureEnv(["DONNA_STATE_DIR", "DONNA_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  let tmpDir: string;
  ...
  beforeEach(async () => {
    ...
    // Set environment variables so resolveDonnaAgentDir() returns mainAgentDir
    process.env.DONNA_STATE_DIR = tmpDir;
    process.env.DONNA_AGENT_DIR = mainAgentDir;
