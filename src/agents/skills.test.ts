...
const fixtureSuite = createFixtureSuite("donna-skills-suite-");
...
beforeAll(async () => {
  ...
  tempHome = await createTempHomeEnv("donna-skills-home-");
  await fs.mkdir(path.join(tempHome.home, ".donna", "agents", "main", "sessions"), {
    recursive: true,
  });
});
...
describe("applySkillEnvOverrides", () => {
  it("sets and restores env vars", async () => {
    ...
    await writeSkill({
      ...
      metadata: '{"donna":{"requires":{"env":["ENV_KEY"]},"primaryEnv":"ENV_KEY"}}',
    });
  ...
  it("blocks unsafe env overrides but allows declared secrets", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata:
        '{"donna":{"requires":{"env":["OPENAI_API_KEY","NODE_OPTIONS"]},"primaryEnv":"OPENAI_API_KEY"}}',
    });
  ...
  it("blocks dangerous host env overrides even when declared", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"donna":{"requires":{"env":["BASH_ENV","SHELL"]}}}',
    });
  ...
  it("allows required env overrides from snapshots", async () => {
    ...
    await writeSkill({
      ...
      description: "Needs env",
      metadata: '{"donna":{"requires":{"env":["OPENAI_API_KEY"]}}}',
    });
