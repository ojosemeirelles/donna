    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "donna.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "donna.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "donna.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("donna-");
  const acpxRoot = await tempDirs.make("donna-acpx-plugin-");
  const helperRoot = await tempDirs.make("donna-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("donna-");
  const pluginRoot = await tempDirs.make("donna-plugin-");
  const outsideDir = await tempDirs.make("donna-outside-");
