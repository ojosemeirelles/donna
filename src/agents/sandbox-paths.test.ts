...
import { resolvePreferredDonnaTmpDir } from "../infra/tmp-donna-dir.js";
...
async function withOutsideHardlinkInDonnaTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside donna tmp root",
      media: path.join(os.tmpdir(), "outside-donna", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Donna tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Donna tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInDonnaTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked Donna tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInDonnaTmp(
