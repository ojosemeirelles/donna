...
async function resolveContext(config: DonnaConfig, sessionKey: string, workspaceDir: string) {
...
function createDefaultsSandboxConfig(scope: "agent" | "shared" | "session" = "agent"): DonnaConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
...
function createWorkSetupCommandConfig(scope: "agent" | "shared"): DonnaConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
