...
import { createDonnaCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "donna-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: DonnaConfig) {
    return createDonnaCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<DonnaConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<DonnaConfig["agents"]>["list"]>[number]["tools"];
  }): DonnaConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createDonnaCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createDonnaCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createDonnaCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createDonnaCodingTools({
      config: cfg,
    ...
    const bobTools = createDonnaCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createDonnaCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createDonnaCodingTools({
      config: cfg,
