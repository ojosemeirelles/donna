...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): DonnaConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as DonnaConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): DonnaConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as DonnaConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: DonnaConfig, accountId = "media") {
    await handleTelegramAction(
