...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    donna: process.env.DONNA_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.donna === undefined) {
      delete process.env.DONNA_GATEWAY_TOKEN;
    } else {
      process.env.DONNA_GATEWAY_TOKEN = envSnapshot.donna;
    }
    if (envSnapshot.donna === undefined) {
      delete process.env.DONNA_GATEWAY_TOKEN;
    } else {
  ...
  it("uses DONNA_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.DONNA_GATEWAY_TOKEN = "local-env-token";
    process.env.DONNA_GATEWAY_TOKEN = "legacy-env-token";
    configState.value = {
      gateway: {
