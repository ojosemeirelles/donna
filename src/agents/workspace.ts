...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.DONNA_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".donna", `workspace-${profile}`);
  }
  return path.join(home, ".donna", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".donna";
