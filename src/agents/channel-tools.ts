...
export function listChannelSupportedActions(params: {
  ...
  const cfg = params.cfg ?? ({} as DonnaConfig);
...
export function listChannelAgentTools(params: { cfg?: DonnaConfig }): ChannelAgentTool[] {
...
function runPluginListActions(plugin: ChannelPlugin, cfg: DonnaConfig): ChannelMessageActionName[] {
  if (!plugin.actions?.listActions) {
    return [];
  }
  try {
