...
describe("logger helpers", () => {
  ...
  it("uses daily rolling default log file and prunes old ones", () => {
    ...
    const todayPath = path.join(DEFAULT_LOG_DIR, `donna-${today}.log`);

    // create an old file to be pruned
    const oldPath = path.join(DEFAULT_LOG_DIR, "donna-2000-01-01.log");
...
function pathForTest() {
  const file = path.join(os.tmpdir(), `donna-log-${crypto.randomUUID()}.log`);
