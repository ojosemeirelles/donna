...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    DONNA_STATE_DIR: "/tmp/donna-daemon",
    DONNA_CONFIG_PATH: "/tmp/donna-daemon/donna.json",
  },
}));
...
const resolveStateDir = vi.fn((env: NodeJS.ProcessEnv) => env.DONNA_STATE_DIR ?? "/tmp/donna-cli");
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.DONNA_CONFIG_PATH ?? `${stateDir}/donna.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/donna-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.DONNA_STATE_DIR = "/tmp/donna-cli";
    process.env.DONNA_CONFIG_PATH = "/tmp/donna-cli/donna.json";
