...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "donna-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "donna-bundled-skills-test-"));
    process.env.DONNA_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
