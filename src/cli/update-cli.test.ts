...
import type { DonnaConfig, ConfigFileSnapshot } from "../config/types.donna.js";
...
vi.mock("../infra/donna-root.js", () => ({
  resolveDonnaPackageRoot: vi.fn(),
}));
...
const { resolveDonnaPackageRoot } = await import("../infra/donna-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/donna-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as DonnaConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/donna-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveDonnaPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("donna-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveDonnaPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("donna");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/donna-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "donna-gateway" }],
      hints: [],
    });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as DonnaConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveDonnaPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("donna-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ DONNA_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.DONNA_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("donna-update-wizard");
    await withEnvAsync({ DONNA_GIT_DIR: tempDir }, async () => {
      setTty(true);
