      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["donna gateway run", "Run the gateway in the foreground."],
            ["donna gateway status", "Show service status and probe reachability."],
            ["donna gateway discover", "Find local and wide-area gateway beacons."],
            ["donna gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.donna.ai/cli/gateway")}\n`,
      ),
  );
