...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Donna Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(donna completion --shell zsh)
  return (
 ...
 * Returns true if profile has `source <(donna completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.donna.ai/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option("--write-state", "Write completion scripts to $DONNA_STATE_DIR/completions (no stdout)")
...
export async function installCompletion(shell: string, yes: boolean, binName = "donna") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c donna -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'donna gateway' and not 'donna other gateway', we need to check the sequence.
