...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      DONNA_GATEWAY_PORT: String(params.port),
      ...(params.token ? { DONNA_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.DONNA_STATE_DIR = "/tmp/donna-cli-state";
    process.env.DONNA_CONFIG_PATH = "/tmp/donna-cli-state/donna.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        DONNA_PROFILE: "dev",
        DONNA_STATE_DIR: "/tmp/donna-daemon-state",
        DONNA_CONFIG_PATH: "/tmp/donna-daemon-state/donna.json",
        DONNA_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.donna.gateway.plist",
    });
