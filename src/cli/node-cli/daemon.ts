...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("donna node install"), formatCliCommand("donna node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("donna node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    DONNA_LOG_PREFIX: baseEnv.DONNA_LOG_PREFIX ?? "node",
