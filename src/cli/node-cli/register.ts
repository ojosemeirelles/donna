    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["donna node run --host 127.0.0.1 --port 18789", "Run the node host in the foreground."],
          ["donna node status", "Check node host service status."],
          ["donna node install", "Install the node host service."],
          ["donna node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.donna.ai/cli/node")}\n`,
    );
