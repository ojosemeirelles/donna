...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Donna and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["donna update", "Update a source checkout (git)"],
        ["donna update --channel beta", "Switch to beta channel (git + npm)"],
        ["donna update --channel dev", "Switch to dev channel (git + npm)"],
        ["donna update --tag beta", "One-off update to a dist-tag or version"],
        ["donna update --dry-run", "Preview actions without changing anything"],
        ["donna update --no-restart", "Update without restarting the service"],
        ["donna update --json", "Output result as JSON"],
        ["donna update --yes", "Non-interactive (accept downgrade prompts)"],
        ["donna update wizard", "Interactive update wizard"],
        ["donna --update", "Shorthand for donna update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run donna update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.donna.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["donna update status", "Show channel + version status."],
          ["donna update status --json", "JSON output."],
          ["donna update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.donna.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
