...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@donna/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@donna/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@donna/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@donna/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @donna/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@donna/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@donna/plugin-alpha@1.2.3",
        installPath: "/tmp/donna/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@donna/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @donna/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@donna/plugin-alpha@latest",
      true,
      "/tmp/donna/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@donna/plugin-alpha@latest",
      installPath: "/tmp/donna/extensions/alpha",
      version: "1.2.3",
