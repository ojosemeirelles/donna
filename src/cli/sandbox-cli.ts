...
const SANDBOX_EXAMPLES = {
  main: [
    ["donna sandbox list", "List all sandbox containers."],
    ["donna sandbox list --browser", "List only browser containers."],
    ["donna sandbox recreate --all", "Recreate all containers."],
    ["donna sandbox recreate --session main", "Recreate a specific session."],
    ["donna sandbox recreate --agent mybot", "Recreate agent containers."],
    ["donna sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["donna sandbox list", "List all sandbox containers."],
    ["donna sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["donna sandbox recreate --all", "Recreate all containers."],
    ["donna sandbox recreate --session main", "Recreate a specific session."],
    ["donna sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["donna sandbox recreate --browser --all", "Recreate only browser containers."],
    ["donna sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["donna sandbox explain", "Show effective sandbox config."],
    ["donna sandbox explain --session agent:main:main", "Explain a specific session."],
    ["donna sandbox explain --agent work", "Explain an agent sandbox."],
    ["donna sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.donna.ai/cli/sandbox")}\n`,
    )
    .action(() => {
