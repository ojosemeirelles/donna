    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["donna nodes status", "List known nodes with live status."],
          ["donna nodes pairing pending", "Show pending node pairing requests."],
          ['donna nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["donna nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.donna.ai/cli/nodes")}\n`,
    );
