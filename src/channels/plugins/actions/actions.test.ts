...
function telegramCfg(): DonnaConfig {
  return { channels: { telegram: { botToken: "tok" } } } as DonnaConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: DonnaConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as DonnaConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as DonnaConfig;
...
function createSignalAccountOverrideCfg(): DonnaConfig {
  return {
    channels: {
      signal: {
...
function createDiscordModerationOverrideCfg(params?: { channelsEnabled?: boolean }): DonnaConfig {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as DonnaConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as DonnaConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as DonnaConfig,
        accountId: undefined,
        params: {
