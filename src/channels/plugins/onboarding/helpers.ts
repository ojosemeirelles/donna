...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in Donna config",
  ...
  const resolved = await promptSecretRefForOnboarding({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "DONNA_SECRET",
      envVarFormatError:
        'Use an env var name like "DONNA_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
...
export async function promptParsedAllowFromForScopedChannel(params: {
  ...
  getExistingAllowFrom: (params: { cfg: DonnaConfig; accountId: string }) => Array<string | number>;
}): Promise<DonnaConfig> {
