...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: DonnaConfig, policy: ChannelAccessPolicy) => DonnaConfig;
  resolveAllowlist: (params: { cfg: DonnaConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: DonnaConfig; resolved: TResolved }) => DonnaConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: DonnaConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: DonnaConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: DonnaConfig, policy: ChannelAccessPolicy): DonnaConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: DonnaConfig, policy: ChannelAccessPolicy): DonnaConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: DonnaConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: DonnaConfig; resolved: string[] }) => {
      calls.push("apply");
