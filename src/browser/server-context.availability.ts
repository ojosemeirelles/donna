...
export function createProfileAvailability({
  ...
  const waitForCdpReadyAfterLaunch = async (): Promise<void> => {
    // launchDonnaChrome() can return before Chrome is fully ready to serve /json/version + CDP WS.
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (!httpReachable) {
      ...
      const launched = await launchDonnaChrome(current.resolved, profile);
      attachRunning(launched);
      try {
        await waitForCdpReadyAfterLaunch();
      } catch (err) {
        await stopDonnaChrome(launched).catch(() => {});
        setProfileRunning(null);
        throw err;
      }
      return;
    ...
    if (!profileState.running) {
      throw new Error(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by donna. ` +
          `Run action=reset-profile profile=${profile.name} to kill the process.`,
      );
    }

    await stopDonnaChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchDonnaChrome(current.resolved, profile);
