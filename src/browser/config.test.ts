...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and donna-orange color", () => {
    ...
    expect(profile?.name).toBe("donna");
    expect(profile?.driver).toBe("donna");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const donna = resolveProfile(resolved, "donna");
    expect(donna?.driver).toBe("donna");
    expect(donna?.cdpPort).toBe(18800);
    expect(donna?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from DONNA_GATEWAY_PORT when unset", () => {
    withEnv({ DONNA_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const donna = resolveProfile(resolved, "donna");
      expect(donna?.cdpPort).toBe(19012);
      expect(donna?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ DONNA_GATEWAY_PORT: undefined }, () => {
      ...
      const donna = resolveProfile(resolved, "donna");
      expect(donna?.cdpPort).toBe(19022);
      expect(donna?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(donna?.cdpPort).toBe(19000);
    expect(donna?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "donna");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        donna: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("donna");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to donna profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps donna default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("donna");
    ...
    it("keeps donna default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("donna");
    ...
    it("keeps donna default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
