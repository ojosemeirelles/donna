...
const RELAY_TOKEN_CONTEXT = "donna-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const DONNA_RELAY_BROWSER = "Donna/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken = process.env.DONNA_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or DONNA_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedDonnaRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === DONNA_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
