import type { BrowserConfig, BrowserProfileConfig, DonnaConfig } from "../config/config.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_DONNA_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "donna" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_DONNA_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_DONNA_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Donna browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for donna/donna CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: DonnaConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_DONNA_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_DONNA_BROWSER_PROFILE_NAME]
        ? DEFAULT_DONNA_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "donna";
