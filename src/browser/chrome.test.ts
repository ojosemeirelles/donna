...
type StopChromeTarget = Parameters<typeof stopDonnaChrome>[0];
...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "donna-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateDonnaProfile(userDataDir, { color: DEFAULT_DONNA_BROWSER_COLOR });
    ...
    expect(def.shortcut_name).toBe(DEFAULT_DONNA_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(path.join(userDataDir, ".donna-profile-decorated"), "utf-8");
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_DONNA_BROWSER_PROFILE_NAME);
  });
});
...
describe("browser chrome helpers", () => {
  ...
  it("stopDonnaChrome no-ops when process is already killed", async () => {
    const proc = makeChromeTestProc({ killed: true });
    await stopChromeWithProc(proc, 10);
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopDonnaChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopDonnaChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
