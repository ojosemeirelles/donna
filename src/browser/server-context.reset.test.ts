...
function localDonnaProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "donna",
...
function createLocalDonnaResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localDonnaProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveDonnaUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "donna-reset-"));
    const profileDir = path.join(tempRoot, "donna");
    ...
    const ops = createLocalDonnaResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveDonnaUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "donna-reset-no-own-"));
    const profileDir = path.join(tempRoot, "donna");
    fs.mkdirSync(profileDir, { recursive: true });
