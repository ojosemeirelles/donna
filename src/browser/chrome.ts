...
export function resolveDonnaUserDataDir(profileName = DEFAULT_DONNA_BROWSER_PROFILE_NAME) {
...
export async function launchDonnaChrome(
  ...
  const userDataDir = resolveDonnaUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_DONNA_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateDonnaProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  try {
    ...
    log.warn(`donna browser clean-exit prefs failed: ${String(err)}`);
  }
...
export async function stopDonnaChrome(running: RunningChrome, timeoutMs = CHROME_STOP_TIMEOUT_MS) {
