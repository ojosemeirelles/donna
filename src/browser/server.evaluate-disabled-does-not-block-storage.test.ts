  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          donna: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.DONNA_GATEWAY_PORT;
    process.env.DONNA_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.DONNA_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.DONNA_GATEWAY_PASSWORD;
