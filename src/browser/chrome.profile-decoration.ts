...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".donna-profile-decorated");
}
...
export function decorateDonnaProfile(
  ...
  const desiredName = opts?.name ?? DEFAULT_DONNA_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_DONNA_BROWSER_COLOR).toUpperCase();
