...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredDonnaTmpDir: vi.fn(() => "/tmp/donna"),
}));
vi.mock("../infra/tmp-donna-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredDonnaTmpDir.mockReturnValue("/tmp/donna");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "donna-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".donna-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredDonnaTmpDir.mockReturnValue("/tmp/donna-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "donna-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "donna-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredDonnaTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "donna-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "donna-preferred", "downloads")}${path.sep}`),
    );
  });
