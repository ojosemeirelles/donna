...
describe("canvas host state dir defaults", () => {
  it("uses DONNA_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("donna-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Donna Canvas");
      } finally {
        await handler.close();
      }
    });
