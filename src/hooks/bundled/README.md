...
This directory contains hooks that ship with Donna. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.donna/donna.json`:
...
2. Restart gateway: `pkill -9 -f 'donna.*gateway' && pnpm donna gateway`
3. Enable the hook: `donna hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.donna.ai/automation/hooks
