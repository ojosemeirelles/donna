...
homepage: https://docs.donna.ai/automation/hooks#command-logger
metadata:
  {
    "donna":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.donna/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.donna/logs/commands.log ~/.donna/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/donna`:
   ```
   /home/username/.donna/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
