...
`;
    ...
    expect(parsed.donna.emoji).toBe("💾");
    expect(parsed.donna.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.donna.emoji).toBe("📝");
    expect(parsed.donna.events).toEqual(["command"]);
    expect(parsed.donna.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.donna.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"donna": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"donna": {"events": ["test"]}}');
  });
...
describe("resolveDonnaMetadata", () => {
  it("extracts donna metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveDonnaMetadata(frontmatter);
  ...
  it("returns undefined when donna key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        donna: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Donna" },
            { id: "npm", kind: "npm", package: "@donna/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@donna/hook");
  });
...
`;
    ...
    const donna = resolveDonnaMetadata(frontmatter);
    expect(donna).toBeDefined();
    expect(donna?.emoji).toBe("💾");
    expect(donna?.events).toEqual(["command:new", "command:reset"]);
    expect(donna?.requires?.config).toEqual(["workspace.dir"]);
    expect(donna?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const donna = resolveDonnaMetadata(frontmatter);
    expect(donna?.emoji).toBe("disk");
    expect(donna?.events).toEqual(["command:new"]);
  });
});
