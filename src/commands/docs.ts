...
const SEARCH_TOOL = "https://docs.donna.ai/mcp.SearchDonna";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.donna.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('donna docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.donna.ai/");
      runtime.log(`Search: ${formatCliCommand('donna docs "your query"')}`);
    }
    return;
  }
