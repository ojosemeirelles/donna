...
import { type DonnaConfig, readConfigFileSnapshot, writeConfigFile } from "../../config/config.js";
...
export async function loadValidConfigOrThrow(): Promise<DonnaConfig> {
...
export async function updateConfig(
  mutator: (cfg: DonnaConfig) => DonnaConfig,
): Promise<DonnaConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: DonnaConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: DonnaConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("donna agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
