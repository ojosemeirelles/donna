...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveDonnaAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("donna models auth setup-token")}\` or \`${formatCliCommand("donna configure")}\`.`
          : `Run \`${formatCliCommand("donna configure")}\` or set an API key env var.`;
