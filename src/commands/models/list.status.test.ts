...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveDonnaAgentDir: vi.fn().mockReturnValue("/tmp/donna-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/donna-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/donna-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveDonnaAgentDir: mocks.resolveDonnaAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/donna-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveDonnaAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/donna-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/donna-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/donna-agent-custom");
