...
export function normalizeCompatibilityConfigValues(cfg: DonnaConfig): {
  config: DonnaConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: DonnaConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as DonnaConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as DonnaConfig["browser"],
    };
    changes.push(
