...
beforeAll(() => {
  envSnapshot = captureEnv(["DONNA_PROFILE"]);
  process.env.DONNA_PROFILE = "isolated";
});
...
vi.mock("../infra/donna-root.js", () => ({
  resolveDonnaPackageRoot: vi.fn().mockResolvedValue("/tmp/donna"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/donna/pnpm-lock.yaml",
      markerPath: "/tmp/donna/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.donna.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.donna.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("donna status --all") ||
          line.includes("donna --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("DONNA_GATEWAY_TOKEN", "abcd1234", async () => {
