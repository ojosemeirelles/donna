...
function resolveAgentName(cfg: DonnaConfig, agentId: string) {
...
function resolveAgentModel(cfg: DonnaConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: DonnaConfig): AgentSummary[] {
