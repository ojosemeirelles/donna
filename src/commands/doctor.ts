...
import { resolveDonnaPackageRoot } from "../infra/donna-root.js";
...
import { noteMemorySchemaVersion, noteMemorySearchHealth } from "./doctor-memory-search.js";
import {
...
function resolveMode(cfg: DonnaConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveDonnaPackageRoot({
  ...
  let cfg: DonnaConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("donna configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("donna config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("donna setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("donna doctor --fix")}" to apply changes.`);
  }
