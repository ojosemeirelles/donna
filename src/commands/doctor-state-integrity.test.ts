...
type EnvSnapshot = {
  HOME?: string;
  DONNA_HOME?: string;
  DONNA_STATE_DIR?: string;
  DONNA_OAUTH_DIR?: string;
};
...
function captureEnv(): EnvSnapshot {
  return {
    ...
    DONNA_OAUTH_DIR: process.env.DONNA_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: DonnaConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: DonnaConfig) {
...
async function runStateIntegrityText(cfg: DonnaConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "donna-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.DONNA_HOME = tempHome;
    process.env.DONNA_STATE_DIR = path.join(tempHome, ".donna");
    delete process.env.DONNA_OAUTH_DIR;
    fs.mkdirSync(process.env.DONNA_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when DONNA_OAUTH_DIR is explicitly configured", async () => {
    process.env.DONNA_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: DonnaConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints donna-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: DonnaConfig = {};
    ...
    expect(text).toMatch(/donna sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/donna sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /donna sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
