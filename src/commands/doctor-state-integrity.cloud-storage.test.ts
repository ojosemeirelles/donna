...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Donna", ".donna");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/donna-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".donna");
  ...
  it("anchors cloud detection to OS homedir when DONNA_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".donna");
    const originalDonnaHome = process.env.DONNA_HOME;
    process.env.DONNA_HOME = "/tmp/donna-home-override";
    ...
    try {
      ...
      if (originalDonnaHome === undefined) {
        delete process.env.DONNA_HOME;
      } else {
        process.env.DONNA_HOME = originalDonnaHome;
      }
    }
  });
