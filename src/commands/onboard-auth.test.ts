...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under DONNA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("donna-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "donna-oauth-sync-"));
    ...
    process.env.DONNA_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "donna-oauth-nosync-"));
    process.env.DONNA_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside DONNA_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "donna-oauth-external-"));
    process.env.DONNA_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* DONNA_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to DONNA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("donna-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
