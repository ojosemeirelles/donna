...
import type { DonnaConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: DonnaConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("donna security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.donna/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("donna approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("donna configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("donna config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("donna doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("donna config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('donna config set session.dmScope "per-channel-peer"') +
