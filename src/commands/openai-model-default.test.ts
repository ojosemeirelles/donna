...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: DonnaConfig },
  primary: string,
) {
...
function expectConfigUnchanged(applied: { changed: boolean; next: DonnaConfig }, cfg: DonnaConfig) {
  expect(applied.changed).toBe(false);
  expect(applied.next).toEqual(cfg);
}

type SharedDefaultModelCase = {
  apply: (cfg: DonnaConfig) => { changed: boolean; next: DonnaConfig };
  defaultModel: string;
  overrideConfig: DonnaConfig;
  alreadyDefaultConfig: DonnaConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: DonnaConfig) => config,
      applyDefaultConfig: (config: DonnaConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: DonnaConfig = { agents: { defaults: {} } };
