...
export const resolveDonnaPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadDonnaPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveDonnaPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.DONNA_STATE_DIR;
  originalUpdateInProgress = process.env.DONNA_UPDATE_IN_PROGRESS;
  process.env.DONNA_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "donna-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.DONNA_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.DONNA_UPDATE_IN_PROGRESS;
  } else {
    process.env.DONNA_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
