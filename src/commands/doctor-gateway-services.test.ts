...
async function runRepair(cfg: DonnaConfig) {
...
function setupGatewayTokenRepairScenario(expectedToken: string) {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      DONNA_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service DONNA_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
  mocks.buildGatewayInstallPlan.mockResolvedValue({
    ...
    environment: {
      DONNA_GATEWAY_TOKEN: expectedToken,
    },
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  ...
  it("uses DONNA_GATEWAY_TOKEN when config token is missing", async () => {
    await withEnvAsync({ DONNA_GATEWAY_TOKEN: "env-token" }, async () => {
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    mocks.findExtraGatewayServices.mockResolvedValue([
      {
        platform: "linux",
        label: "donna-gateway.service",
        detail: "unit: /home/test/.config/systemd/user/donna-gateway.service",
        scope: "user",
        legacy: true,
      },
    ]);
    mocks.uninstallLegacySystemdUnits.mockResolvedValue([
      {
        name: "donna-gateway",
        unitPath: "/home/test/.config/systemd/user/donna-gateway.service",
        enabled: true,
        exists: true,
      },
    ]);
    ...
    expect(mocks.note).toHaveBeenCalledWith(
      expect.stringContaining("donna-gateway.service"),
      "Legacy gateway removed",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing Donna gateway next.",
    );
  });
});
