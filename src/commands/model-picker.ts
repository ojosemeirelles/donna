...
type PromptDefaultModelResult = { model?: string; config?: DonnaConfig };
...
function resolveConfiguredModelRaw(cfg: DonnaConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: DonnaConfig): string[] {
...
export function applyPrimaryModel(cfg: DonnaConfig, model: string): DonnaConfig {
...
export function applyModelAllowlist(cfg: DonnaConfig, models: string[]): DonnaConfig {
  const defaults = cfg.agents?.defaults;
