...
export function isCommandEnabled(cfg: DonnaConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: DonnaConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: DonnaConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: DonnaConfig) {
