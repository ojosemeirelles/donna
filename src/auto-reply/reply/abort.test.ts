...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "donna-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop donna!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@donna_bot", { botUsername: "donna_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@donna_bot", { botUsername: "donna_bot" })).toBe(true);
