...
function createDiscordParams(commandBody: string, cfg: DonnaConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: DonnaConfig = baseCfg) {
  const params = createDiscordParams(commandBody, cfg);
  params.ctx.MessageThreadId = defaultThreadId;
  return params;
}

async function runDiscordAcpCommand(commandBody: string, cfg: DonnaConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: DonnaConfig = baseCfg) {
