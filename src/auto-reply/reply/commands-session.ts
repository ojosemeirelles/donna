...
import { scheduleGatewaySigusr1Restart, triggerDonnaRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Donna in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerDonnaRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Donna via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
