...
import type { DonnaConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareDonnaVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  donna config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  donna config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: DonnaConfig,
  pathSegments: string[],
): { changed: boolean; next: DonnaConfig } {
...
function coerceConfig(value: unknown): DonnaConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as DonnaConfig;
}
...
function stampConfigVersion(cfg: DonnaConfig): DonnaConfig {
...
function warnIfConfigFromFuture(cfg: DonnaConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareDonnaVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Donna (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as DonnaConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): DonnaConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as DonnaConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: DonnaConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as DonnaConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.DONNA_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.DONNA_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.DONNA_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.DONNA_WATCH_SESSION === "string" &&
        deps.env.DONNA_WATCH_SESSION.trim().length > 0
          ? deps.env.DONNA_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.DONNA_WATCH_COMMAND === "string" &&
        deps.env.DONNA_WATCH_COMMAND.trim().length > 0
          ? deps.env.DONNA_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `DONNA_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: DonnaConfig | null = null;
let runtimeConfigSourceSnapshot: DonnaConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.DONNA_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.DONNA_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function setRuntimeConfigSnapshot(config: DonnaConfig, sourceConfig?: DonnaConfig): void {
...
export function getRuntimeConfigSnapshot(): DonnaConfig | null {
