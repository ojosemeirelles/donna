...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "donna.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    ...
    DONNA_PLUGIN_MANIFEST_CACHE_MS: process.env.DONNA_PLUGIN_MANIFEST_CACHE_MS,
  };

  const validateInSuite = (raw: unknown) => validateConfigObjectWithPlugins(raw);

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "donna-config-plugin-validation-"));
    ...
    process.env.DONNA_STATE_DIR = path.join(suiteHome, ".donna");
    process.env.DONNA_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.DONNA_STATE_DIR === undefined) {
      delete process.env.DONNA_STATE_DIR;
    } else {
      process.env.DONNA_STATE_DIR = envSnapshot.DONNA_STATE_DIR;
    }
    if (envSnapshot.DONNA_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.DONNA_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.DONNA_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.DONNA_PLUGIN_MANIFEST_CACHE_MS;
    }
  });
