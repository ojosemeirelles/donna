...
import type { ConfigFileSnapshot } from "./types.donna.js";
import { DonnaSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(DonnaSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.donna/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/donna/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/donna/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/donna/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/donna/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = DonnaSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "DonnaConfig";
    const hints = mainSchemaHints;
