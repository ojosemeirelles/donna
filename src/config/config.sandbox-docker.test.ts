...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("donna-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "donna-sandbox-browser-global" },
      agentBrowser: { network: "donna-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("donna-sandbox-browser-agent");
  });
