...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "donna-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ DONNA_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-donna",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-donna#general");
  });
  ...
  it("derives session transcripts dir from DONNA_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { DONNA_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when DONNA_STATE_DIR differs from stored paths", () => {
