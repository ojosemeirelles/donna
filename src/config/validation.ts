...
import type { DonnaConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: DonnaConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: DonnaConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: DonnaConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = DonnaSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as DonnaConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as DonnaConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as DonnaConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as DonnaConfig,
  };
}
