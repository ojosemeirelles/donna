...
// Prefix for Donna-generated action IDs to scope our handler
const DONNA_ACTION_PREFIX = "donna:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  // Handle Block Kit button clicks from Donna-generated messages
  // Only matches action_ids that start with our prefix to avoid interfering
  // with other Slack integrations or future features
  ctx.app.action(new RegExp(`^${DONNA_ACTION_PREFIX}`), async (args: SlackActionMiddlewareArgs) => {
  ...
  const modalMatcher = new RegExp(`^${DONNA_ACTION_PREFIX}`);

  // Handle Donna modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
