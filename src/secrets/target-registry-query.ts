...
const DONNA_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
  (entry) => entry.configFile === "donna.json",
);
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of DONNA_COMPILED_SECRET_TARGETS) {
...
const DONNA_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargets(config: DonnaConfig): DiscoveredConfigSecretTarget[] {
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries =
    allowedTargetIds === null
      ? DONNA_COMPILED_SECRET_TARGETS
      : Array.from(allowedTargetIds).flatMap((targetId) => DONNA_TARGETS_BY_ID.get(targetId) ?? []);
