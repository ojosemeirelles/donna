...
const { makeStorePath } = createCronStoreHarness({ prefix: "donna-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses DONNA_HOME for tilde expansion", () => {
    vi.stubEnv("DONNA_HOME", "/srv/donna-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/donna-home", "cron", "jobs.json"));
  });
});
