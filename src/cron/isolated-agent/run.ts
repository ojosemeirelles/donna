...
export type RunCronAgentTurnResult = {
   ...
   * messages.  See: https://github.com/donna/donna/issues/15692
   */
  delivered?: boolean;
  /**
...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.DONNA_TEST_FAST === "1";
  ...
  // correct per-agent paths (e.g. ~/.donna/agents/<agentId>/agent/).
  ...
  const cfgWithAgentDefaults: DonnaConfig = {
    ...params.cfg,
    ...
    const fallbackResult = await runWithModelFallback({
      ...
      run: async (providerOverride, modelOverride) => {
        ...
        if (isCliProvider(providerOverride, cfgWithAgentDefaults)) {
          ...
          // the configured timeoutSeconds. See: https://github.com/donna/donna/issues/29774
