...
let originalDonnaLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.DONNA_TEST_FILE_LOG;
  originalDonnaLogLevel = process.env.DONNA_LOG_LEVEL;
  delete process.env.DONNA_TEST_FILE_LOG;
  delete process.env.DONNA_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.DONNA_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalDonnaLogLevel === undefined) {
    delete process.env.DONNA_LOG_LEVEL;
  } else {
    process.env.DONNA_LOG_LEVEL = originalDonnaLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
