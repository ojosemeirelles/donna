...
function listAgents(cfg: DonnaConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: DonnaConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<DonnaConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: DonnaConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: DonnaConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: DonnaConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
  byChannelAccountIndex: Map<string, EvaluatedBindingsIndex>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<DonnaConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  DonnaConfig,
  {
    bindingsRef: DonnaConfig["bindings"];
    agentsRef: DonnaConfig["agents"];
    sessionRef: DonnaConfig["session"];
...
function resolveRouteCacheForConfig(cfg: DonnaConfig): Map<string, ResolvedAgentRoute> {
