...
export async function handleWhatsAppStart(host: DonnaApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: DonnaApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: DonnaApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: DonnaApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: DonnaApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: DonnaApp): string {
...
function resolveGatewayHttpAuthHeader(host: DonnaApp): string | null {
...
function buildGatewayHttpHeaders(host: DonnaApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: DonnaApp) {
...
export function handleNostrProfileToggleAdvanced(host: DonnaApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: DonnaApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: DonnaApp) {
  const state = host.nostrProfileFormState;
