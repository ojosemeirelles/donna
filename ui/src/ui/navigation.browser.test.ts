...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/donna/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/donna");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/donna/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__DONNA_CONTROL_UI_BASE_PATH__ = "/donna";
    const app = mountApp("/donna/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/donna");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/donna/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem("donna.control.settings.v1", JSON.stringify({ token: "existing-token" }));
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
