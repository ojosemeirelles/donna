...
const shouldBuild = (deps) => {
  if (deps.env.DONNA_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.DONNA_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[donna] ${message}\n`);
};

const runDonna = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["donna.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
