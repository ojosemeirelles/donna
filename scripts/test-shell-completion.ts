 ...
 * `donna update`. Use it to verify the completion installation flow
...
\`donna update\`, \`donna doctor\`, and \`donna onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Donna Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`donna completion --install\` later to enable.`));
    return;
  }
