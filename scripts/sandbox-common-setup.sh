...
BASE_IMAGE="${BASE_IMAGE:-donna-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-donna-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=donna.sandbox=1)
NOTE
