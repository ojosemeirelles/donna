#!/bin/bash
# Setup Donna Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'donna-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Donna message: Send warning via Donna itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/donna-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/donna-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/donna-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now donna-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status donna-auth-monitor.timer"
echo ""
echo "Quick commands:"
