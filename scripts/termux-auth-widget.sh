...
# Check auth status
termux-toast "Checking Donna auth..."

STATUS=$(ssh "$SERVER" '$HOME/donna/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/donna/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|DONNA_EXPIRING|DONNA_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/donna/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|DONNA_EXPIRED|DONNA_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Donna won't work until re-auth"
                ;;
        esac
        ;;
