...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/donna/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Donna Re-Auth" -c "After getting key, run: ssh $SERVER '~/donna/scripts/mobile-reauth.sh'" --id donna-auth
        ;;
    *)
