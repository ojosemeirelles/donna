#!/usr/bin/env bash
# ClawDock - Docker helpers for Donna
# Inspired by Simon Willison's "Running Donna in Docker"
# https://til.simonwillison.net/llms/donna-docker
#
# Installation:
...
# Common paths to check for Donna
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/donna"
  "${HOME}/workspace/donna"
  "${HOME}/projects/donna"
  "${HOME}/dev/donna"
  "${HOME}/code/donna"
  "${HOME}/src/donna"
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Donna at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/donna"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Donna not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/donna/donna.git ~/donna"
    echo "  cd ~/donna && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^DONNA_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d donna-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart donna-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f donna-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec donna-gateway \
    bash -c 'echo "alias donna=\"./donna.mjs\"" > /tmp/.bashrc_donna && bash --rcfile /tmp/.bashrc_donna'
}
...
clawdock-cli() {
  _clawdock_compose run --rm donna-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build donna-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "DONNA_GATEWAY_TOKEN=$token" donna-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" donna-gateway \
    bash -c './donna.mjs config set gateway.remote.token "$TOKEN" && ./donna.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec donna-gateway \
    bash -c "./donna.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart donna-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm donna-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec donna-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'donna config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Donna${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (donna alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to donna project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.donna)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'donna channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'donna status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.donna.ai${_CLR_RESET}"
  echo ""
}
