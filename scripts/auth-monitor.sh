...
#
# Suggested cron: */30 * * * * /home/admin/donna/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Donna notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., donna-alerts)
...
STATE_FILE="$HOME/.donna/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Donna if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use donna
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Donna to $NOTIFY_PHONE..."
            donna send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Donna is down. Run: ssh l36 '~/donna/scripts/mobile-reauth.sh'" "urgent"
    exit 1
