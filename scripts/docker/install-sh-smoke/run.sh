...
INSTALL_URL="${DONNA_INSTALL_URL:-https://donna.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${DONNA_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${DONNA_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="donna"
PACKAGE_NAME="${DONNA_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (DONNA_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${DONNA_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${DONNA_INSTALL_LATEST_OUT:-}"
fi
