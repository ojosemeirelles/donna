...
# Build and bundle Donna into a minimal .app we can open.
# Outputs to dist/Donna.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Donna.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Donna"
BUNDLE_ID="${BUNDLE_ID:-ai.donna.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/donna/donna/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Donna/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :DonnaBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :DonnaGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Donna"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Donna"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Donna"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Donna" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Donna/Resources/Donna.icns" "$APP_ROOT/Contents/Resources/Donna.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Donna/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying DonnaKit resources"
DONNAKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/DonnaKit_DonnaKit.bundle"
if [ -d "$DONNAKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/DonnaKit_DonnaKit.bundle"
  cp -R "$DONNAKIT_BUNDLE" "$APP_ROOT/Contents/Resources/DonnaKit_DonnaKit.bundle"
else
  echo "WARN: DonnaKit resource bundle not found at $DONNAKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Donna"
killall -q Donna 2>/dev/null || true
