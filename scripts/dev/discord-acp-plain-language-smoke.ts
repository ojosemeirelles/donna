...
type DriverMode = "token" | "webhook" | "donna";
...
function resolveStateDir(): string {
  const override = process.env.DONNA_STATE_DIR?.trim() || process.env.DONNA_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.DONNA_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".donna");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver donna] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Donna spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|donna> Driver transport mode (default: token)\n" +
    ...
    "  --donna-bin <path>        Donna CLI binary for driver=donna (default: donna)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  DONNA_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  DONNA_DISCORD_SMOKE_DRIVER\n" +
    "  DONNA_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  DONNA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  DONNA_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  DONNA_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  DONNA_DISCORD_SMOKE_AGENT\n" +
    "  DONNA_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  DONNA_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  DONNA_DISCORD_SMOKE_POLL_MS\n" +
    "  DONNA_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  DONNA_DISCORD_SMOKE_DONNA_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.DONNA_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.DONNA_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.DONNA_DISCORD_SMOKE_DRIVER ||
    process.env.DONNA_DISCORD_SMOKE_DRIVER ||
    "token";
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.DONNA_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.DONNA_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.DONNA_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.DONNA_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DONNA_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.DONNA_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.DONNA_DISCORD_SMOKE_AGENT ||
    process.env.DONNA_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.DONNA_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.DONNA_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.DONNA_DISCORD_SMOKE_INSTRUCTION ||
    process.env.DONNA_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.DONNA_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.DONNA_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.DONNA_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const donnaBin =
    resolveArg("--donna-bin") || process.env.DONNA_DISCORD_SMOKE_DONNA_BIN || "donna";
...
async function donnaCliJson<T>(params: { donnaBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.donnaBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`donna ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await donnaCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    donnaBin: params.donnaBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "donna") {
    return await readMessagesWithOpenclaw({
      donnaBin: params.args.donnaBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `donna-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("donna message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Donna.`,
        diagnostics: {
          bindingCandidates: [
            {
