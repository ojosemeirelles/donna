...
const unitIsolatedFilesRaw = [
  ...
  "src/agents/donna-tools.subagents.sessions-spawn.lifecycle.test.ts",
...
// DONNA_TEST_VM_FORKS=0, and let users force-enable with =1.
const supportsVmForks = Number.isFinite(nodeMajor) ? nodeMajor !== 24 : true;
const useVmForks =
  process.env.DONNA_TEST_VM_FORKS === "1" ||
  (process.env.DONNA_TEST_VM_FORKS !== "0" && !isWindows && supportsVmForks && !lowMemLocalHost);
const disableIsolation = process.env.DONNA_TEST_NO_ISOLATE === "1";
const includeGatewaySuite = process.env.DONNA_TEST_INCLUDE_GATEWAY === "1";
const includeExtensionsSuite = process.env.DONNA_TEST_INCLUDE_EXTENSIONS === "1";
const runs = [
  ...(useVmForks
    ? [
        {
...
const shardOverride = Number.parseInt(process.env.DONNA_TEST_SHARDS ?? "", 10);
...
const shardIndexOverride = (() => {
  const parsed = Number.parseInt(process.env.DONNA_TEST_SHARD_INDEX ?? "", 10);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : null;
})();

if (shardIndexOverride !== null && shardCount <= 1) {
  console.error(
    `[test-parallel] DONNA_TEST_SHARD_INDEX=${String(
      shardIndexOverride,
    )} requires DONNA_TEST_SHARDS>1.`,
  );
  process.exit(2);
}
...
if (shardIndexOverride !== null && shardIndexOverride > shardCount) {
  console.error(
    `[test-parallel] DONNA_TEST_SHARD_INDEX=${String(
      shardIndexOverride,
    )} exceeds DONNA_TEST_SHARDS=${String(shardCount)}.`,
  );
  process.exit(2);
}
const windowsCiArgs = isWindowsCi ? ["--dangerouslyIgnoreUnhandledErrors"] : [];
const silentArgs = process.env.DONNA_TEST_SHOW_PASSED_LOGS === "1" ? [] : ["--silent=passed-only"];
const rawPassthroughArgs = process.argv.slice(2);
const passthroughArgs =
  rawPassthroughArgs[0] === "--" ? rawPassthroughArgs.slice(1) : rawPassthroughArgs;
const rawTestProfile = process.env.DONNA_TEST_PROFILE?.trim().toLowerCase();
const testProfile =
  rawTestProfile === "low" ||
  rawTestProfile === "max" ||
...
const maxOldSpaceSizeMb = (() => {
  ...
  const raw = process.env.DONNA_TEST_MAX_OLD_SPACE_SIZE_MB ?? "";
