...
IMAGE_NAME="donna-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="donna-net-e2e-$$"
GW_NAME="donna-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "DONNA_GATEWAY_TOKEN=$TOKEN" \
  -e "DONNA_SKIP_CHANNELS=1" \
  -e "DONNA_SKIP_GMAIL_WATCHER=1" \
  -e "DONNA_SKIP_CRON=1" \
  -e "DONNA_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
