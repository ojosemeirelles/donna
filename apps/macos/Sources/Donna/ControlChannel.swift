...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: DonnaProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.donna", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: DonnaKit.AnyCodable]()) {
                $0[$1.key] = DonnaKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: DonnaProtocol.AnyCodable?) -> [String: DonnaProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: DonnaProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: DonnaKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: DonnaProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("donna.control.heartbeat")
    static let controlAgentEvent = Notification.Name("donna.control.agent")
}
