...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.donna/donna.json. Prefer DONNA_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.donna/donna.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.donna/donna.json. Restart the gateway to apply."
        }
