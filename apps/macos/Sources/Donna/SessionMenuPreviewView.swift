...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.donna", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> DonnaSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: DonnaSessionPreviewEntry,
    ...
    private static func cache(payload: DonnaSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: DonnaChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [DonnaKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [DonnaKit.AnyCodable]) -> [DonnaChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(DonnaChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: DonnaChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: DonnaChatMessage) -> Bool {
    ...
    private static func toolNames(for message: DonnaChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: DonnaChatMessage) -> String? {
