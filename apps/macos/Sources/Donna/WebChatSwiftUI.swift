...
private let webChatSwiftLogger = Logger(subsystem: "ai.donna", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: DonnaChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> DonnaChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(DonnaChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> DonnaChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                DonnaGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<DonnaChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any DonnaChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = DonnaChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: DonnaChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<DonnaChatView>) -> NSViewController
    {
