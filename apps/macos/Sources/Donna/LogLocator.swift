...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["DONNA_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/donna")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("donna-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("donna-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/donna/ (rolling or stdout), or nil if none exist.
