...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.donna.mac`) even if the shipped
    ...
    {
        if environment["DONNA_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "donna.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "donna.nixMode") { return true }
