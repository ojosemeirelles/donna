...
enum DonnaLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = DonnaOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = DonnaFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        DonnaLogging.bootstrapIfNeeded()
        let label = DonnaLogging.makeLabel(subsystem: subsystem, category: category)
...
struct DonnaOSLogHandler: AppLogLevelBackedHandler {
...
struct DonnaFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = DonnaLogging.parseLabel(self.label)
