...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.donna", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: DonnaNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [DonnaCapability.canvas.rawValue, DonnaCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(DonnaCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if DonnaLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(DonnaCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            DonnaSystemCommand.which.rawValue,
            DonnaSystemCommand.run.rawValue,
            DonnaSystemCommand.execApprovalsGet.rawValue,
            DonnaSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(DonnaCapability.camera.rawValue) {
        ...
        if capsSet.contains(DonnaCapability.location.rawValue) {
