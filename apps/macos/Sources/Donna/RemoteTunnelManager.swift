...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.donna", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Donna run already has an SSH listener on the expected port (common after restarts),
