...
enum CommandResolver {
    private static let projectRootDefaultsKey = "donna.gatewayProjectRootPath"
    private static let helperName = "donna"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let donnaEntry = root.appendingPathComponent("donna.mjs").path
        if FileManager().isReadableFile(atPath: donnaEntry) { return donnaEntry }
        let binEntry = root.appendingPathComponent("bin/donna.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/donna")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let donnaPaths = self.donnaManagedPaths(home: home)
        if !donnaPaths.isEmpty {
            extras.insert(contentsOf: donnaPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + donnaPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func donnaManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".donna"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func donnaExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectDonnaExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("donna.mjs").path,
            root.appendingPathComponent("bin/donna.js").path,
        ]
    ...
    static func hasAnyDonnaInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.donnaExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let donnaPath = self.projectDonnaExecutable(projectRoot: root) {
            return [donnaPath, subcommand] + extraArgs
        }
        if let donnaPath = self.donnaExecutable(searchPaths: searchPaths) {
            return [donnaPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "donna", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            donna entrypoint missing (looked for dist/index.js or donna.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real donna CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/donna"
        ...
        \(cliSection)
        \(projectSection)
        if command -v donna >/dev/null 2>&1; then
          CLI="$(command -v donna)"
          donna \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/donna.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/donna.mjs"
            node "$PRJ/donna.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/donna.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/donna.js"
            node "$PRJ/bin/donna.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent donna"
          pnpm --silent donna \(quotedArgs);
        else
          echo "donna CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? DonnaConfigFile.loadDict()
