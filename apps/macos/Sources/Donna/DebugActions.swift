...
enum DebugActions {
    private static let verboseDefaultsKey = "donna.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "donna.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = DonnaPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Donna", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = DonnaPaths.configURL
        guard
