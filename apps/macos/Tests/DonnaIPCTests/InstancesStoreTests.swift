...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: DonnaProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: DonnaProtocol.AnyCodable] = [
            "presence": .init([DonnaProtocol.AnyCodable(entry)]),
        ]
        let payload = DonnaProtocol.AnyCodable(payloadMap)
