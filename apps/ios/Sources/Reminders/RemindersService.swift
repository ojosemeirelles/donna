...
final class RemindersService: RemindersServicing {
    func list(params: DonnaRemindersListParams) async throws -> DonnaRemindersListPayload {
        ...
        let payload: [DonnaReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return DonnaRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: DonnaRemindersAddParams) async throws -> DonnaRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = DonnaReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return DonnaRemindersAddPayload(reminder: payload)
    }
