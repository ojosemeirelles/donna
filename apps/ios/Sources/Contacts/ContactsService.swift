...
final class ContactsService: ContactsServicing {
    ...
    func search(params: DonnaContactsSearchParams) async throws -> DonnaContactsSearchPayload {
        ...
        return DonnaContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: DonnaContactsAddParams) async throws -> DonnaContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return DonnaContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return DonnaContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> DonnaContactPayload {
        DonnaContactPayload(
