...
@MainActor
final class DonnaAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.donna.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.donna.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.donna.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "donna.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "donna.watch.promptId"
    static let sessionKeyKey = "donna.watch.sessionKey"
    static let actionPrimaryIDKey = "donna.watch.action.primary.id"
    static let actionPrimaryLabelKey = "donna.watch.action.primary.label"
    static let actionSecondaryIDKey = "donna.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "donna.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "donna.watch.action.primary"
    static let actionSecondaryIdentifier = "donna.watch.action.secondary"
    static let actionIdentifierPrefix = "donna.watch.action."
    static let actionIDKeyPrefix = "donna.watch.action.id."
    static let actionLabelKeyPrefix = "donna.watch.action.label."
    static let categoryPrefix = "donna.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: DonnaWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> DonnaWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return DonnaWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Donna" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [DonnaWatchAction]) -> [UNNotificationAction] {
...
struct DonnaApp: App {
    ...
    @UIApplicationDelegateAdaptor(DonnaAppDelegate.self) private var appDelegate
...
extension DonnaApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Donna: installing uncaught exception handler")
