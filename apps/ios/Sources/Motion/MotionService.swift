...
final class MotionService: MotionServicing {
    func activities(params: DonnaMotionActivityParams) async throws -> DonnaMotionActivityPayload {
        ...
        let mapped: [DonnaMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return DonnaMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: DonnaPedometerParams) async throws -> DonnaPedometerPayload {
        ...
        let payload: DonnaPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = DonnaPedometerPayload(
