...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__DONNA_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<DonnaTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.donna", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = DonnaTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> DonnaTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return DonnaTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> DonnaTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> DonnaTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return DonnaTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> DonnaTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = DonnaTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: DonnaTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: DonnaAgentEventPayload.self
            ) else {
                continue
            }
