...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> DonnaDeviceStatusPayload {
        ...
        return DonnaDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    private func batteryStatus() -> DonnaBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: DonnaBatteryState = switch device.batteryState {
        ...
        return DonnaBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> DonnaThermalStatusPayload {
        let state: DonnaThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return DonnaThermalStatusPayload(state: state)
    }

    private func storageStatus() -> DonnaStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return DonnaStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
