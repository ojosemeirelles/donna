...
struct IOSGatewayChatTransport: DonnaChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.donna", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> DonnaChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(DonnaChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> DonnaChatHistoryPayload {
        ...
        return try JSONDecoder().decode(DonnaChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [DonnaChatAttachmentPayload]) async throws -> DonnaChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [DonnaChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(DonnaChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(DonnaGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: DonnaGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: DonnaChatEventPayload.self)
                        {
