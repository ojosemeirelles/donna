...
private let chatUILogger = Logger(subsystem: "ai.donna", category: "DonnaChatUI")

@MainActor
@Observable
public final class DonnaChatViewModel {
    public private(set) var messages: [DonnaChatMessage] = []
    ...
    public var attachments: [DonnaPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [DonnaChatPendingToolCall] = []
    public private(set) var sessions: [DonnaChatSessionEntry] = []
    private let transport: any DonnaChatTransport
    ...
    private var pendingToolCallsById: [String: DonnaChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any DonnaChatTransport) {
    ...
    public var sessionChoices: [DonnaChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [DonnaChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: DonnaPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [DonnaChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: DonnaChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: DonnaChatMessage) -> DonnaChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> DonnaChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return DonnaChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: DonnaChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [DonnaChatMessage],
        incoming: [DonnaChatMessage]) -> [DonnaChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [DonnaChatMessage]) -> [DonnaChatMessage] {
        var result: [DonnaChatMessage] = []
    ...
    private static func dedupeKey(for message: DonnaChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [DonnaChatMessageContent] = [
            DonnaChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> DonnaChatAttachmentPayload in
            DonnaChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> DonnaChatSessionEntry {
        DonnaChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: DonnaChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: DonnaChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: DonnaAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = DonnaChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> DonnaPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
