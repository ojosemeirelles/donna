...
@MainActor
struct DonnaChatComposer: View {
    @Bindable var viewModel: DonnaChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(DonnaChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(DonnaChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \DonnaPendingAttachment.id)
                { (att: DonnaPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            DonnaPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(DonnaChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(DonnaChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(DonnaChatTheme.subtleCard)
        .clipShape(Capsule())
    }
