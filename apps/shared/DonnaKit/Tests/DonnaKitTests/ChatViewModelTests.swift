...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> DonnaChatHistoryPayload
{
    DonnaChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> DonnaChatSessionEntry {
...
private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [DonnaChatHistoryPayload],
    sessionsResponses: [DonnaChatSessionsListResponse] = []) async -> (TestChatTransport, DonnaChatViewModel)
{
    let transport = TestChatTransport(historyResponses: historyResponses, sessionsResponses: sessionsResponses)
    let vm = await MainActor.run { DonnaChatViewModel(sessionKey: sessionKey, transport: transport) }
    return (transport, vm)
}

private func loadAndWaitBootstrap(
    vm: DonnaChatViewModel,
...
private func sendUserMessage(_ vm: DonnaChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, DonnaChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [DonnaChatHistoryPayload]
    private let sessionsResponses: [DonnaChatSessionsListResponse]

    private let stream: AsyncStream<DonnaChatTransportEvent>
    private let continuation: AsyncStream<DonnaChatTransportEvent>.Continuation

    init(
        historyResponses: [DonnaChatHistoryPayload],
        sessionsResponses: [DonnaChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<DonnaChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> DonnaChatHistoryPayload {
        ...
        return self.historyResponses.last ?? DonnaChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [DonnaChatAttachmentPayload]) async throws -> DonnaChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return DonnaChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> DonnaChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? DonnaChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: DonnaChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = DonnaChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func stripsInboundMetadataFromHistoryMessages() async throws {
        let history = DonnaChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [
                AnyCodable([
...
"""]],
        ...
        let vm = await MainActor.run { DonnaChatViewModel(sessionKey: "main", transport: transport) }
