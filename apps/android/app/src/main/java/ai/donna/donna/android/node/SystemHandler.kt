...
private const val NOTIFICATION_CHANNEL_BASE_ID = "donna.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Donna Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Donna Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Donna Active")
      }
